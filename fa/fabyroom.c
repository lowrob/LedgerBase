...
fabyroom()
{
	...
	arayptr[1] = (char *)&fatype ;
	arayptr[2] = (char *)&sch_rec ;
	arayptr[3] = NULL ;
	...
	/* Initialize Rec. Entry Header Record to start reading form the
 	...
 	fa_rec.fa_curcostcen = cc_nbr1;
	fa_rec.fa_roomno[0] = '\0';
	fa_rec.fa_itemid = 0;
	flg_reset( FAMAST );

	for( ; ; ) {
		code = get_n_famast(&fa_rec,BROWSE,4,FORWARD,e_mesg);
		if( code < 0) {
			if(code == EFL) break ;
			code = DBH_ERR;
			break ;
		}
		if (fa_rec.fa_curcostcen > cc_nbr2 )
			break;

		if (fa_rec.fa_curcostcen != sch_rec.sc_numb) {
			sch_rec.sc_numb = fa_rec.fa_curcostcen ;
			code = get_sch(&sch_rec,BROWSE,0,e_mesg);
			if( code < 0) 
				STRCPY(fatype.desc, "NOT DEFINED");
		}

		switch( fa_rec.fa_cond[0] ){
			case CD_EXCELLENT:
				STRCPY(fa_rec.fa_suppname,EXCELLENT);
				break;
			case CD_GOOD:
				STRCPY(fa_rec.fa_suppname,GOOD);
				break;
			case CD_FAIR:
				STRCPY(fa_rec.fa_suppname,FAIR);
				break;
			case CD_POOR:
				STRCPY(fa_rec.fa_suppname,POOR);
				break;
			case CD_OBSOLETE:
				STRCPY(fa_rec.fa_suppname,OBSOLETE);
				break;
		}
		STRCPY( fatype.code, fa_rec.fa_type );
		code = get_fatype( &fatype, BROWSE, 0, e_mesg );
		if( code < 0) 
			STRCPY(fatype.desc, "NOT DEFINED");
