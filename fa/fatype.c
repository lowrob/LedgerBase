...
/* fatype.sth - header for C structure generated by PROFOM EDITOR */
...
Fa_type		fatype, oldfatype;	/* Fixed Asset types file */
Pa_rec		pa_rec;
...
Validate()	/* Validate the values entered by the user */
{
	...
	switch( sr.curfld ){
		case 500:	/* Code */
			if( ESC_H ){
				retval = fatype_hlp( s_rec.s_code, 7,13 );
				if( retval==DBH_ERR )
					return( retval );
				else if( retval>=0 )
					redraw();
			...
			STRCPY( fatype.code,s_rec.s_code );
			index = get_fatype( &fatype, BROWSE, 0, e_mesg );
			if( index==ERROR ){
				fomen(e_mesg); get();
				return(-1);
			}
...
int	mode;		/* ADD, UPDATE */
{
	STRCPY( fatype.desc, s_rec.s_desc );
	retval = put_fatype( &fatype, mode, e_mesg );
	if( retval!=NOERROR ){
		fomen(e_mesg);get();
		roll_back(e_mesg);
		return(retval);
	}
	err = rite_audit((char*)&s_rec, FATYPE,mode,(char *)&fatype,
				(char *)&oldfatype,e_mesg);
	if(err==LOCKED) {
		DispError(e_mesg);
		roll_back(e_mesg);
		return(LOCKED);
...
{
	STRCPY( s_rec.s_desc, fatype.desc );
...
int	direction;
{
	if( flg_start(FATYPE)!=direction ){ 	/* file access mode changed */
		STRCPY(fatype.code, s_rec.s_code) ;
		inc_str( fatype.code, (sizeof(s_rec.s_code)-1), direction );
		flg_reset(FATYPE);
	}
	/* Read the next record from stmast file */
	retval = get_n_fatype( &fatype, BROWSE, 0, direction, e_mesg );
#ifndef ORACLE
	seq_over( FATYPE );
#endif
	if( retval==EFL ){
	...
	STRCPY( s_rec.s_code, fatype.code );
...
GetRecord()	/* Read the header key values, get terminal info ,read rec */
{
	...
	if( (retval=ReadFields(500,500))<0 || retval==ESCAPE ) 
		return( retval );
	STRCPY( fatype.code, s_rec.s_code );
	retval = get_fatype( &fatype, mode, 0, e_mesg );
	if( retval!=NOERROR ){
		fomer(e_mesg);
		get();
		return(retval);
...
ChangeRecord()	/* Change an existing record */
{
	...
	scpy( (char *)&oldfatype, (char *)&fatype, sizeof( Fa_type ) );
	...
	for( ; ; ) {
		...
		if(err==LOCKED) {
			roll_back(e_mesg);
			STRCPY( fatype.code, s_rec.s_code );
			retval = get_fatype( &fatype, UPDATE, 0, e_mesg );
			if(retval < 0){
				fomen(e_mesg);
				get() ;
