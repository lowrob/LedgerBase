/*-----------------------------------------------------------------------
Source Name: ded_cat.c
System     : Personnel/Payroll System.
Created  On: 16th Oct. 91.
Created  By: Andre Cormier 

DESCRIPTION:
	Program To input the Deduction Category Codes.
...
#define MAINFL		DED_CAT			/* main file used */

#define	SYSTEM		"SETUP"			/* Sub System Name */
#define	MOD_DATE	"16-OCT-91"		/* Progran Last Modified */
...
#define	SCR_NAME	"ded_cat"	/* PROFOM screen Name */
...
#define	DED_FLD		1000	/* Deduction Code Field */
...
typedef	struct	{	/* Start Fld 900, Endfld 5300 and Step 300 */
	...
	char	s_ded_code[7]; 	/* 1000-5200 STRING X(7) */
	char	s_ded_desc[31];	/* 1100-5300 STRING X(30) */
...
typedef struct	{
	...
	char	s_ded_cat[7];	/* 500 STRING X(6) */
...
static	Ded_cat	ded_cat, pre_ded_cat;	/* Deduction Category record */
static	Deduction deduction;		/* Deduction record */
static	Pay_per		pay_per;	/* Pay Period Record */
...
int	direction ;
{
	...
	strcpy(ded_cat.dc_cat_code,s_sth.s_ded_cat);
	ded_cat.dc_code[0] = '\0';
	ded_cat.dc_pp_code[0] = '\0';
	inc_str(ded_cat.dc_cat_code, sizeof(ded_cat.dc_cat_code)-1,
			direction);
	flg_reset(DED_CAT);

	err = get_n_ded_cat(&ded_cat,BROWSE,0,direction,e_mesg);
	if(err < 0 && err != EFL) {
		DispError((char *)&s_sth,e_mesg);
		return(err);
	}
	...
	if(err == EFL) {
		...
		strcpy(s_sth.s_ded_cat,ded_cat.dc_cat_code);
		strcpy(s_sth.s_pp_code,ded_cat.dc_pp_code);
...
ReadKey()
{
	...
	strcpy(cat_code,s_sth.s_ded_cat);
	strcpy(pp_code,s_sth.s_pp_code);
	s_sth.s_ded_cat[0] = LV_CHAR;
	...
	if(RET_USER_ESC == i){
		strcpy(s_sth.s_ded_cat,cat_code);
...
int curr_item;
{
	...
	for(a = 0 ; a < PAGESIZE ; a++) {
		...
		if(s_sth.s_items[a].s_ded_code == LV_CHAR) break;

		if(strcmp(s_sth.s_items[curr_item].s_ded_code,s_sth.s_items[a].
		s_ded_code)==0) {
			return(-1);
		}
	}
...
BuildList()
{
	...
	for( ; ; ) {
		retval = get_n_ded_cat(&ded_cat,BROWSE,0,FORWARD,e_mesg);
		if(retval < 0) {
			if(retval == EFL) break;
			DispError((char *)&s_sth,e_mesg);
			return(retval);
		...
		if(strcmp(ded_cat.dc_cat_code,s_sth.s_ded_cat)!=0) { 
			break;
		}
		...
		strcpy(deduction.dd_code, ded_cat.dc_code);
		strcpy(deduction.dd_pp_code, ded_cat.dc_pp_code);
		retval = get_deduction(&deduction,BROWSE,0,e_mesg);
		if(retval < 0) {
			if(retval == EFL)
				strcpy(CurPage->Items[i].s_ded_desc,"Deduction Code Not Setup"); 
			else {	
				DispError((char *)&s_sth,e_mesg);
				return(retval);
			}
			...
			strcpy(CurPage->Items[i].s_ded_desc,deduction.dd_desc);

		strcpy(CurPage->Items[i].s_ded_code,ded_cat.dc_code);
...
int	mode;
{
	...
	strcpy(ded_cat.dc_cat_code,s_sth.s_ded_cat) ;
	strcpy(ded_cat.dc_code,temppage->Items[item_no].s_ded_code) ;
	strcpy(ded_cat.dc_pp_code,s_sth.s_pp_code) ;
	if(mode != ADD) {
		scpy((char *)&pre_ded_cat,(char *)&ded_cat,
			sizeof(pre_ded_cat));

		retval = get_ded_cat(&ded_cat,UPDATE,0,e_mesg) ;
		if(retval < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(retval);
	...
	retval = put_ded_cat(&ded_cat,mode,e_mesg) ;
	if(retval < 0) {
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(retval);
	...
	if(mode != ADD) {
		retval = rite_audit((char*)&s_sth,DED_CAT,mode,(char*)&ded_cat,
			(char*)&pre_ded_cat,e_mesg);
		if(retval==LOCKED) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg) ;
			return(LOCKED) ;
...
int	mode ;
{
	...
	s_sth.s_items[item_no].s_ded_code[0] = LV_CHAR;
	strcpy(s_sth.s_items[item_no].s_status,ACTIVE);

	SetDupBuffers(DED_FLD+(STEP*item_no), DED_FLD+(STEP*item_no),1);
...
Validation()
{
	...
	case KEY_START:
		...
		if(s_sth.s_fn[0] != ADDREC) {
			if(retval < 0) {
				fomer("Deduction Category Code Does not Exist");
				s_sth.s_ded_cat[0] = LV_CHAR;
				return(ERROR);
			}
			if(strcmp(ded_cat.dc_cat_code,s_sth.s_ded_cat) != 0) {
			...
			if(retval >= 0) {
				if(strcmp(s_sth.s_ded_cat,ded_cat.dc_cat_code) == 0) {
					fomer("Deduction Category Code Already Exist");
					s_sth.s_ded_cat[0] = LV_CHAR;
					return(ERROR);
				}
			}
	...
	case	KEY_END:
		...
		break;
	case	DED_FLD:
		strcpy(deduction.dd_code,s_sth.s_items[item_no].s_ded_code);
		strcpy(deduction.dd_pp_code,s_sth.s_pp_code);
		retval = get_deduction(&deduction,BROWSE,0,e_mesg);
		if(retval < 0) {
		...
		if(item_no != 0) {
			...
			if(retval < 0){
				fomer("Deduction Code Already Listed, Please Re-Enter");
				s_sth.s_items[item_no].s_ded_code[0] = LV_CHAR;
				return(ERROR);
			}
		}
		strcpy(s_sth.s_items[item_no].s_ded_desc, deduction.dd_desc);
		if ( WriteFields((char *)&s_sth,DESC_FLD+(STEP*item_no), 
...
WindowHelp()
{
	...
	switch(fld_no){
	case KEY_START:
		retval = ded_cat_hlp(s_sth.s_ded_cat,s_sth.s_pp_code,7,13);
		if(retval == DBH_ERR) return(retval);
		if(s_sth.s_fn[0] != ADDREC) {
			if(retval >= 0) {
				redraw();
				if(s_sth.s_ded_cat[0] == '\0') {
					s_sth.s_ded_cat[0] = LV_CHAR;
					return(ERROR);
				}
	...
	case KEY_END:
		...
		break;
	case	DED_FLD:
		retval = deduction_hlp(s_sth.s_items[item_no].s_ded_code,
...
int	status;
{
	...
	DispMesgFld((char *)&s_sth); ;
		...
		if(s_sth.s_items[s_sth.s_field -1].s_ded_code[0] != HV_CHAR) {	
...
char	t_char ;
{
	s_sth.s_items[item_no].s_ded_code[0] = t_char ;
	s_sth.s_items[item_no].s_ded_desc[0] = t_char ;
