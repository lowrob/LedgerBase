...
*	Source 	: classlist.c 
*
*	Program to Print classification codes using REPORT GENERATOR.
*
...
short	err;
	...
	date1 = 0;
	date2 = get_date();
	if((retval = GetDaRange(&date1,&date2))<0)
		return(retval);
	...
	strcpy( code1, "     1");
	strcpy( code2, "ZZZZZZ" );
	retval = GetClassRange( code1,code2 );
	if(retval < 0) return(retval);
	...
	for( ; ; ) {
		code = get_n_class(&class,BROWSE,1,FORWARD,e_mesg);
		...
		if((class.c_date < date1) || 
		   (class.c_date > date2))	{
			continue;
		}

		if(strcmp(class.c_pos,posi1) < 0 ||
		   strcmp(class.c_pos,posi2) > 0) {
			continue;
		}

		if(strcmp(class.c_code,code1) < 0 ||
		   strcmp(class.c_code,code2) > 0) {
			continue;
		}
