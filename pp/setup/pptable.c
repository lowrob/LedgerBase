...
*	Source 	: pptable.c 
*
*	Program to Print Pay Period Table using REPORT GENERATOR.
*	By : Andre Cormier
...
short	err;
	...
	for( ; ; ) {
		...
		strcpy(pay_per_it.ppi_code,pay_period.pp_code);
		pay_per_it.ppi_year = pay_period.pp_year;
		pay_per_it.ppi_numb = 0;
		pay_per_it.ppi_st_date = 0;

		flg_reset(PAY_PER_ITEM);

		for(;;){
			code = get_n_pp_it(&pay_per_it,BROWSE,0,
			       FORWARD,e_mesg);

			if(strcmp(pay_per_it.ppi_code,pay_period.pp_code) != 0 
			|| pay_per_it.ppi_year != pay_period.pp_year
			|| code == EFL ) {
				break;
			}
	...
	close_file(PAY_PERIOD) ;
	close_file(PAY_PER_ITEM) ;
	rpclose();
