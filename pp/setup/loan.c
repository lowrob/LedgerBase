...
Created  On: 26th Sept. 91.
Created  By: C. Burns.    

DESCRIPTION:
	The CSB/Loan Maintenance Screen is used to identify the 
...
#define	MOD_DATE	"26-SEPT-91"		/* Progran Last Modified */
...
#define	SCR_NAME	"loan"	/* PROFOM screen Name */
...
#define	CODE_FLD	800	/* CSB/Loan Code */
#define DESC_FLD	900	/* Description field */
#define FUND_FLD	1000	/* Fund Field */
#define AMT_ACCT	1100	/* Amount G/L Account */
#define INT_ACCT	1200	/* Interest G/L Account */

#define STATUS_FLD	500	/* Status Field */

#define PAGE_FLD	700	/* Page# Field */

#define	ITEM_ST_FLD	800	/* Item Start Field */
#define ITEM_END_FLD	3100	/* Item End Field */
#define	STEP		600	/* # of Fields on one line */

#define	END_FLD		3300	/* Last Field of the screen */
...
typedef	struct	{	/* Start Fld 1500, Endfld 4200 */
	...
	short	s_fund;		/* 1000 NUMERIC 999 */
	char	s_amt_acct[19];	/* 1100 STRING X(18) */
	char	s_int_acct[19];	/* 1200 STRING X(18) */
...
int	mode;
{
	...
	strcpy(loan.cs_code,s_sth.s_items[0].s_code);
	err = get_loan(&loan,mode,0,e_mesg);
	if(err < 0) {
		DispError((char *)&s_sth,e_mesg);
		return(err);
	}
...
BuildList()
{
	...
	for( ; ; ) {
		retval = get_n_loan(&loan,BROWSE,0,FORWARD,e_mesg);
		if(retval < 0) {
			if(retval == EFL) break;
			DispError((char *)&s_sth,e_mesg);
			return(retval);
		...
		strcpy(CurPage->Items[i].s_code, loan.cs_code);
		strcpy(CurPage->Items[i].s_desc, loan.cs_desc);
		CurPage->Items[i].s_fund = loan.cs_fund;
		strcpy(CurPage->Items[i].s_amt_acct, loan.cs_amt_acct);
		strcpy(CurPage->Items[i].s_int_acct, loan.cs_int_acct);
...
int	mode;
{
	...
	scpy((char *)&pre_loan,(char *)&loan,sizeof(loan));
...
int	mode;
{
	...
	strcpy(loan.cs_code,temppage->Items[item_no].s_code);
	if(mode != ADD) {
		retval = get_loan(&loan,UPDATE,0,e_mesg) ;
		if(retval < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(retval);
	...
	strcpy(loan.cs_desc,temppage->Items[item_no].s_desc);
	loan.cs_fund = temppage->Items[item_no].s_fund;
	strcpy(loan.cs_amt_acct,temppage->Items[item_no].s_amt_acct);
	strcpy(loan.cs_int_acct,temppage->Items[item_no].s_int_acct);

	retval = put_loan(&loan,mode,e_mesg) ;
	if(retval < 0) {
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(retval);
	...
	if(mode != ADD) {
		retval = rite_audit((char*)&s_sth,LOAN,mode,(char*)&loan,
			(char*)&pre_loan,e_mesg);
		if(retval==LOCKED) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg) ;
			return(LOCKED) ;
...
Validation()
{
	...
	case CODE_FLD: 
		...
		strcpy(loan.cs_code,s_sth.s_items[item_no].s_code);
		flg_reset(LOAN);
		retval = get_loan(&loan,BROWSE,0,e_mesg);
		if(s_sth.s_fn[0] == ADDREC) {
			if(retval < 0) {
				if (retval == UNDEF) break;
			}
			else {
				fomer("CSB/Loan Code Already Exists");
	...
	case DESC_FLD:
		...
		break;
	case FUND_FLD:
		if(CheckFund(s_sth.s_items[item_no].s_fund)<0) {
			s_sth.s_items[item_no].s_fund = LV_SHORT;
			return(ERROR);
		}
		break;
	case AMT_ACCT:
		if(s_sth.s_items[item_no].s_amt_acct[0] != '\0') {
			if(acnt_chk(s_sth.s_items[item_no].s_amt_acct) < 0)  {
				fomer("Invalid Account Number");
				s_sth.s_items[item_no].s_amt_acct[0] = LV_CHAR;
				return(ERROR);
			}
			if(pa_rec.pa_glmast[0] == YES) {
			   gl_rec.funds = s_sth.s_items[item_no].s_fund;
			   strcpy(gl_rec.accno,s_sth.s_items[item_no].s_amt_acct);
			   gl_rec.reccod = 99;
		...
		break;
	case INT_ACCT:
		if(s_sth.s_items[item_no].s_int_acct[0] != '\0') {
			if(acnt_chk(s_sth.s_items[item_no].s_int_acct) < 0)  {
				fomer("Invalid Account Number");
				s_sth.s_items[item_no].s_int_acct[0] = LV_CHAR;
				return(ERROR);
			}
			if(pa_rec.pa_glmast[0] == YES) {
			   gl_rec.funds = s_sth.s_items[item_no].s_fund;
			   strcpy(gl_rec.accno,s_sth.s_items[item_no].s_int_acct);
			   gl_rec.reccod = 99;
			   retval = get_gl(&gl_rec,BROWSE,0,e_mesg);
			   if(retval < 0) {
				fomer(e_mesg);
...
WindowHelp()
{
	...
	case CODE_FLD:
/*
		retval = loan_hlp(s_sth.s_code,7,13);
		if(retval == DBH_ERR) return(retval);
		if(retval >= 0) redraw();
		if(retval == 0) return(ERROR);
		if(retval < 0) return(ERROR);
		strcpy(loan.cs_code,s_sth.s_code);
		retval = get_loan(&loan,BROWSE,0,e_mesg);
		if(retval < 0) {
			fomer(e_mesg);
...
*/
	case AMT_ACCT:
		if(pa_rec.pa_glmast[0] == YES) {
			retval = gl_hlp(s_sth.s_items[0].s_fund,
				s_sth.s_items[0].s_amt_acct,&reccod, 7, 13 );
			if(retval == DBH_ERR) return(retval) ;
			if(retval >=0 ) redraw();
...
#endif
				s_sth.s_items[0].s_amt_acct[0] = LV_CHAR ;
				return(ERROR) ;
			}
			gl_rec.funds = s_sth.s_items[0].s_fund;
			strcpy(gl_rec.accno,s_sth.s_items[0].s_amt_acct);
	...
	case INT_ACCT:
		if(pa_rec.pa_glmast[0] == YES) {
			retval = gl_hlp(s_sth.s_items[0].s_fund,
				s_sth.s_items[0].s_int_acct,&reccod, 7, 13 );
			if(retval == DBH_ERR) return(retval) ;
			if(retval >=0 ) redraw();
			if(retval < 1) return(ERROR) ;	/* Not Selected */
			if(reccod != 99) {
				...
				fomer("Choisir les fiches avec 99 seulement comme code de fiche");
#endif
				s_sth.s_items[0].s_int_acct[0] = LV_CHAR ;
				return(ERROR) ;
			}
			gl_rec.funds = s_sth.s_items[0].s_fund;
			strcpy(gl_rec.accno,s_sth.s_items[0].s_int_acct);
			gl_rec.reccod = reccod;
			retval = get_gl(&gl_rec,BROWSE,0,e_mesg);
			if(retval < 0) {
				fomer(e_mesg);
...
short	item_no ;
{
	...
	s_sth.s_items[item_no].s_amt_acct[0] = t_char ;
	s_sth.s_items[item_no].s_int_acct[0] = t_char ;
