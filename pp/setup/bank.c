...
Module     : Personnel Payroll system.
Created  On: Nov 25, 1991.
Created  By: Andre Cormier 

DESCRIPTION:
	Program to Maintain Bank File.  Program has Add, Change, Delete,
...
#define MAINFL		BANK		/* main file used */

#define	SYSTEM		"SETUP"		/* Sub System Name */
#define	MOD_DATE	"25-NOV-91"	/* Program Last Modified */
...
#define	SCR_NAME	"bank"         /* First screen  */

/* PROFOM Field Numbers */

#define	FN_FLD		400	/* Fn: */
#define	KEY_START	500	/* School file key, School Number */
#define	CHG_FLD		600     /* Field: */
#define BANK_NAME_FLD	800     /* Bank Name */
#define MAIL_ADD_FLD	900     /* Bank Name */
#define STREET_ADD_FLD	1000    /* Bank Name */
#define CITY_FLD	1100    /* Bank Name */
#define PROV_FLD	1200    /* Bank Name */
#define POSTAL_FLD	1300    /* Bank Name */
#define CONTACT_FLD	1400    /* Bank Name */
#define TELEPHONE_FLD	1500    /* Bank Name */
...
#define	LAST_FLD_SCR    8	/* Last Field Number on Screen */
...
typedef	struct{
	...
	char	s1_numb_bank[13]; 	/* NUMERIC 99         Field 500 */
	short	s1_field;         	/* NUMERIC 99         Field 700 */
	char	s1_name_bank[31];	/* STRING X(28)       Field 800 */
	...
	char	s1_add4[31];		/* Address line 3 field 1100 */
	char	s1_pc[8];		/* postal code field 1200 */
	char	s1_contact[31];		/* Contact Person field 1300 */
...
static	s1_struct  s1_sth;	/* Screen Structure */

static	struct  stat_rec sr;		/* PROFOM status rec */

static	Emp	emp;
static	Bank	bank, pre_bank;
...
ReadKey()
{
	...
	if(s1_sth.s1_fn[0] == ADDREC)	/* ADD */
		fomca1(KEY_START,19,0);    /* Off Dup Control */
	else {
	...
	strcpy(key_numb_bank,s1_sth.s1_numb_bank) ;
		...
		if(sr.retcode == RET_USER_ESC){
			if(sr.escchar[0] == 'F' || sr.escchar[0] == 'f') {
				/* copy back key fields */
                		strcpy(s1_sth.s1_numb_bank,key_numb_bank) ;
...
int	fld_no ;
{
	...
	case	KEY_START:
		...
		strcpy(bank.bk_numb,s1_sth.s1_numb_bank);

		flg_reset(BANK);

		err = get_n_bank(&bank,BROWSE,0,FORWARD,e_mesg);

		if(s1_sth.s1_fn[0] == ADDREC) {
			if(strcmp(s1_sth.s1_numb_bank,bank.bk_numb) == 0 && 
			err !=EFL){
				fomen("Bank Number Already Exist");
				s1_sth.s1_numb_bank[0] = LV_CHAR;
				return(-1);
			}
		}
		else {
			if(strcmp(s1_sth.s1_numb_bank,bank.bk_numb) != 0 ||
			err==EFL){
				fomen("Bank Number Does Not Exist");
				s1_sth.s1_numb_bank[0] = LV_CHAR;
				return(-1);
			}
		}
		if(s1_sth.s1_fn[0] == DELETE) {
			...
			for(;;) {
				err = get_n_employee(&emp,BROWSE,0,FORWARD,e_mesg);
				if(err == EFL)
					break;

				if(strcmp(emp.em_bank,s1_sth.s1_numb_bank)==0){
					s1_sth.s1_numb_bank[0] = LV_CHAR;
					fomca1(KEY_START,19,0); 
					fomen("Can't Delete, Bank Number Already Exist");
					return(-1);
				}
			}
		}
	...
	case	BANK_NAME_FLD:		/* School name	*/
		break ;
...
int	md ;
{
	...
	err = rite_audit((char*)&s1_sth,BANK, md, (char*)&bank, (char*)&pre_bank, e_mesg);
...
int md; /* BROWSE or UPDATE */
{
	...
	return(get_bank(&bank, md, 0, e_mesg));
}
/*-----------------------------------------------------------------------*/ 
...
int	direction;
{
	...
	if(flg_start(BANK) != direction) {
		inc_str(bank.bk_numb, sizeof(bank.bk_numb)-1, 
			direction);

		flg_reset(BANK) ;
	}

	err = get_n_bank(&bank, BROWSE, 0, direction, e_mesg);
...
{
	/* Copy screen fields */
	strcpy(s1_sth.s1_numb_bank,bank.bk_numb) ;
	strcpy(s1_sth.s1_name_bank, bank.bk_name) ;
	strcpy(s1_sth.s1_add1, bank.bk_add1) ;
	strcpy(s1_sth.s1_add2, bank.bk_add2) ;
	strcpy(s1_sth.s1_add3, bank.bk_add3) ;
	strcpy(s1_sth.s1_add4, bank.bk_add4) ;
	strcpy(s1_sth.s1_pc, bank.bk_pc) ;
	strcpy(s1_sth.s1_contact, bank.bk_cont) ;
	strcpy(s1_sth.s1_phone, bank.bk_phone) ;
...
int	md ;
{
	/* Save the schl_rec in pre_rec to write audit records for changed
	   fields */
	if(md != ADD)
		scpy((char*)&pre_bank, (char*)&bank, sizeof(Bank)) ;

 	strcpy (bank.bk_numb, s1_sth.s1_numb_bank) ;
	strcpy (bank.bk_name, s1_sth.s1_name_bank) ;
	strcpy (bank.bk_add1, s1_sth.s1_add1) ;
	strcpy (bank.bk_add2, s1_sth.s1_add2) ;
	strcpy (bank.bk_add3, s1_sth.s1_add3) ;
	strcpy (bank.bk_add4, s1_sth.s1_add4) ;
	strcpy (bank.bk_pc, s1_sth.s1_pc) ;
	strcpy (bank.bk_cont, s1_sth.s1_contact) ;
	strcpy (bank.bk_phone, s1_sth.s1_phone) ;
}
/*------------------------------------------------------------------------*/
