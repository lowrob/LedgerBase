...
#define MAINFL		POSITION		/*main file used */
...
#define	SCR_NAME	"position"	/* PROFOM screen Name */
...
#define CHG_FLD		600
#define COLHDR1_FLD	800	/* Desc. Field */
#define COLHDR2_FLD	900	/* Desc. Field */

#define	CODE_FLD	100	/* CSB/Loan Code */
#define DESC_FLD	200	/* Description field */
#define POSTYP_FLD	300	/* Description field */
#define CTRTYP_FLD	400	/* Description field */
#define CTRLGTH_FLD	500	/* Description field */
#define STATUS_FLD	600	/* Status Field */


#define	ITEM_ST_FLD	1000	/* Item Start Field */
#define ITEM_END_FLD	10700	/* Item End Field */
#define	STEP		700	/* # of Fields on one line */

#define	END_FLD		10900	/* Last Field of the screen */
...
static	Position	position,	 pre_position;
...
Change()
{
	...
	ret(WriteFields((char*)&s_sth,COLHDR1_FLD,COLHDR2_FLD));
...
AddItems()
{
	...
	if(CurLast != NULL && CurLast->NoItems < PAGESIZE ) {
		...
		if(CurLast != NULL) {
			...
			s_sth.s_dummy2[0] = ' ';
			s_sth.s_page = 1 ;
			WriteFields((char*)&s_sth,COLHDR1_FLD,COLHDR2_FLD);
			i = 0 ;
		}
	}
...
int	mode;
{
	...
	strcpy(position.p_code,s_sth.s_items[0].s_code);
	err = get_position(&position,mode,0,e_mesg);
	if(err < 0) {
		DispError((char *)&s_sth,e_mesg);
		return(err);
	}
...
BuildList()
{
	...
	for( ; ; ) {
		...
		strcpy(CurPage->Items[i].s_code, position.p_code);
		strcpy(CurPage->Items[i].s_desc,  position.p_desc);
		strcpy(CurPage->Items[i].s_postyp,  position.p_type);
		strcpy(CurPage->Items[i].s_ctrtyp,  position.p_ctr_type);
		CurPage->Items[i].s_ctrlgth =  position.p_ctr_lgth;
...
int	mode;
{
	...
	scpy((char *)&pre_position,(char *)&position,sizeof(position));
...
int	mode;
{
	...
	strcpy(position.p_code,temppage->Items[item_no].s_code);
	if(mode != ADD) {
		retval = get_position(&position,UPDATE,0,e_mesg) ;
		if(retval < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(retval);
	...
	strcpy(position.p_desc,temppage->Items[item_no].s_desc);
	strcpy(position.p_type,temppage->Items[item_no].s_postyp);
	strcpy(position.p_ctr_type,temppage->Items[item_no].s_ctrtyp);
	position.p_ctr_lgth = temppage->Items[item_no].s_ctrlgth ;

	retval = put_position(&position,mode,e_mesg) ;
	if(retval < 0) {
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(retval);
	...
	if(mode != ADD) {
		retval = rite_audit((char*)&s_sth,POSITION,mode,(char*)&position,
			(char*)&pre_position,e_mesg);
		if(retval==LOCKED) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg) ;
			return(LOCKED) ;
...
int	mode ;
{
	...
	s_sth.s_items[item_no].s_postyp[0] = LV_CHAR;
	s_sth.s_items[item_no].s_ctrtyp[0] = LV_CHAR;
	s_sth.s_items[item_no].s_ctrlgth = LV_SHORT;
	if(mode == ADD) {
...
Validation()
{
	...
	case POSTYP_FLD :	/* Position Type Field	*/
		if (s_sth.s_items[item_no].s_postyp[0] == NULL){ 
#ifdef ENGLISH
			fomen("Must be FT, PT, SU, CA");
#else
			fomen("Doit etre PT, TP, SU, TE");
#endif
			s_sth.s_items[item_no].s_postyp[0] = LV_CHAR ;
			return(ERROR) ;
		}
		if(strcmp(s_sth.s_items[item_no].s_postyp,FT) != 0 &&
		strcmp(s_sth.s_items[item_no].s_postyp,PT) != 0 &&
		strcmp(s_sth.s_items[item_no].s_postyp,CA) != 0 &&	
		strcmp(s_sth.s_items[item_no].s_postyp,SU) != 0 ){
#ifdef ENGLISH
	sprintf(e_mesg,	"Invalide Position Type, Please Re-enter");
#else
	sprintf(e_mesg, "Type de position invalide, SVP reseille");
#endif
			DispError((char *)&s_sth,e_mesg);
			s_sth.s_items[item_no].s_postyp[0] = LV_CHAR;
			return(ERROR);	
	...
	case CTRTYP_FLD :	
/*
		strcpy(contract.ctr_type, s_sth.s_items[item_no].s_status);

		retval = get_contract(&contract,BROWSE,0,e_mesg); 

		if(ERROR == err) return(DBH_ERR) ;
		if(EFL == err) break ;
		if(retval == UNDEF){
#ifdef ENGLISH
	sprintf(e_mesg,	"Contract Code Does Not Exist, Please Re-enter");
#else
	sprintf(e_mesg, "Code de contract n'existe pas, SVP reseille");
#endif
			DispError((char *)&s_sth,e_mesg);
			s_sth.s_items[item_no].s_ctrtyp[0] = LV_CHAR;
			return(ERROR);	
		...
		break;
	case CTRLGTH_FLD :	/* Contract Length Field	*/
		if(s_sth.s_items[item_no].s_ctrlgth > 12 ){
#ifdef ENGLISH
	sprintf(e_mesg,	"Invalide Length of Contract, Please Re-enter");
#else
	sprintf(e_mesg, "Duree du contrat invalide, SVP reseille");
#endif
			DispError((char *)&s_sth,e_mesg);
			s_sth.s_items[item_no].s_ctrlgth = LV_SHORT ;
			return(ERROR);	
...
char	*code;
{
	...
	if(retval < 0) {
		...
		fomer("Position Code Already Exists on File");
		return(ERROR);
	}
	   ...
	   for(temppage=FirstPage; temppage!=NULL;temppage=temppage->NextPage) {
	      for(i =0; i< temppage->NoItems; i++) {
			...
			if(strcmp(temppage->Items[i].s_code,code) == 0) {
				fomer("Position Code Already Entered on Screen");
				return(ERROR);
			}
	      }
	      if(temppage == CurLast) break;
...
WindowHelp()
{
	...
	switch(sr.curfld){
	case CODE_FLD :
		retval = position_hlp( s_sth.s_items[item_no].s_code,
...
ClearScreen()
{
	...
	ret( WriteFields((char *)&s_sth,COLHDR1_FLD, (END_FLD - 200)) );
...
short	item_no ;
{
	...
	s_sth.s_items[item_no].s_postyp[0] = t_char;
	s_sth.s_items[item_no].s_ctrtyp[0] = t_char;
	s_sth.s_items[item_no].s_ctrlgth = t_short;
	if(t_char == HV_CHAR) {
...
#else
	sprintf(e_mesg, "Code de position existe deja, SVP reseille");
#endif
	DispError((char *)&s_sth,e_mesg);
	return(ERROR);
}
