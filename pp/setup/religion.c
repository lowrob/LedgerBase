...
#define MAINFL		RELIGION		/*main file used */
...
#define	SCR_NAME	"religion"	/* PROFOM screen Name */
...
#define	CODE_FLD	900	/* CSB/Loan Code */
#define DESC_FLD	1000	/* Description field */
...
static	Religion	religion,	 pre_religion;
...
BuildList()
{
	...
	religion.rel_code[0] = '\0';	
	flg_reset(RELIGION);

	for( ; ; ) {
		retval = get_n_rel(&religion,BROWSE,0,FORWARD,e_mesg);
		if(retval < 0) {
			if(retval == EFL) break;
			DispError((char *)&s_sth,e_mesg);
			return(retval);
		...
		strcpy(CurPage->Items[i].s_code, religion.rel_code);
		strcpy(CurPage->Items[i].s_desc, religion.rel_desc);
...
Page	*pageptr ;
{
	...
	ret( WriteFields((char *)&s_sth,(CODE_FLD-100), (END_FLD - 200)) );
...
int	mode;
{
	...
	scpy((char *)&pre_religion,(char *)&religion,sizeof(religion));
...
int	mode;
{
	...
	strcpy(religion.rel_code,temppage->Items[item_no].s_code);
	if(mode != ADD) {
		retval = get_rel(&religion,UPDATE,0,e_mesg) ;
		if(retval < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(retval);
	...
	strcpy(religion.rel_desc,temppage->Items[item_no].s_desc);

	retval = put_rel(&religion,mode,e_mesg) ;
	if(retval < 0) {
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(retval);
	...
	if(mode != ADD) {
		retval = rite_audit((char*)&s_sth,RELIGION,mode,(char*)&religion,
			(char*)&pre_religion,e_mesg);
		if(retval==LOCKED) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg) ;
			return(LOCKED) ;
...
int	mode ;
{
	...
	if(mode == ADD) {
		SetDupBuffers((ITEM_ST_FLD - 100), END_FLD - 200,0);
	}
	else {
...
Validation()
{
	...
	case CODE_FLD :			/* Religion code field */
...
char	*code;
{
	...
	if(retval < 0) {
		...
		fomer("Religion Code Already Exists on File");
		return(ERROR);
	}
	   ...
	   for(temppage=FirstPage; temppage!=NULL;temppage=temppage->NextPage) {
	      for(i =0; i< temppage->NoItems; i++) {
			...
			if(strcmp(temppage->Items[i].s_code,code) == 0) {
				fomer("Religion Code Already Entered on Screen");
				return(ERROR);
			}
	      }
	      if(temppage == CurLast) break;
...
WindowHelp()
{
	...
	switch(sr.curfld){
	case CODE_FLD :
		retval = rel_hlp( s_sth.s_items[item_no].s_code,
...
#ifdef ENGLISH
	sprintf(e_mesg,	"Religion Code Already Exists, Please Re-enter");
#else
	sprintf(e_mesg, "Code de religion existe deja, SVP reseille");
#endif
	DispError((char *)&s_sth,e_mesg);
	return(ERROR);
}
