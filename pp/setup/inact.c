...
------------------------------------------------------------------------*/
#define	MAIN		MAIN	/* Main program. This is to declare Switches */
#define MAINFL		INACT_CODE	/*main file used */

#define	SYSTEM		"SETUP"			/* Sub System Name */
#define	MOD_DATE	"25-NOV-91"		/* Progran Last Modified */
...
#define	SCR_NAME	"inact"	/* PROFOM screen Name */
...
static	Inact	inactivation,	 pre_inactivation;
...
int	mode;
{
	...
	strcpy(inactivation.i_code,s_sth.s_items[0].s_code);
	err = get_inact(&inactivation,mode,0,e_mesg);
	if(err < 0) {
		DispError((char *)&s_sth,e_mesg);
		return(err);
	}
...
BuildList()
{
	...
	inactivation.i_code[0] = '\0';	
	flg_reset(INACT_CODE);

	for( ; ; ) {
		retval = get_n_inact(&inactivation,BROWSE,0,FORWARD,e_mesg);
		if(retval < 0) {
			if(retval == EFL) break;
			DispError((char *)&s_sth,e_mesg);
			return(retval);
		...
		strcpy(CurPage->Items[i].s_code, inactivation.i_code);
		strcpy(CurPage->Items[i].s_desc, inactivation.i_desc);
...
int	mode;
{
	...
	scpy((char *)&pre_inactivation,(char *)&inactivation,sizeof(inactivation));
...
int	mode;
{
	...
	strcpy(inactivation.i_code,temppage->Items[item_no].s_code);
	if(mode != ADD) {
		retval = get_inact(&inactivation,UPDATE,0,e_mesg) ;
		if(retval < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(retval);
	...
	strcpy(inactivation.i_desc,temppage->Items[item_no].s_desc);

	retval = put_inact(&inactivation,mode,e_mesg) ;
	if(retval < 0) {
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(retval);
	...
	if(mode != ADD) {
		retval = rite_audit((char*)&s_sth,DEPARTMENT,mode,(char*)&inactivation,
			(char*)&pre_inactivation,e_mesg);
		if(retval==LOCKED) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg) ;
			return(LOCKED) ;
...
Validation()
{
	...
	case CODE_FLD :			/* Inactivation code field */
	  strcpy(inactivation.i_code,s_sth.s_items[item_no].s_code);

	/*  Verify Inactivation code is not already present in 
	    Inactivationfile.					*/

	  retval = get_inact(&inactivation,BROWSE,0,e_mesg);
	...
	/*   Verify that the inactivation code is not already present in the
	     linked list.						*/
...
#ifdef ENGLISH
	sprintf(e_mesg,	"Inactivation Code Already Exists, Please Re-enter");
#else
	sprintf(e_mesg, "Code de inactivation existe deja, SVP reseille");
#endif
	DispError((char *)&s_sth,e_mesg);
	return(ERROR);
}
