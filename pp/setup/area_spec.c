...
#define MAINFL		AREA_SPEC		/*main file used */

#define	SYSTEM		"SETUP"			/* Sub System Name */
#define	MOD_DATE	"26-FEB-92"		/* Progran Last Modified */
...
#define	SCR_NAME	"area_spec"	/* PROFOM screen Name */
...
static	Area_spec	area_spec,	 pre_area_spec;
...
int	mode;
{
	...
	strcpy(area_spec.ar_code,s_sth.s_items[0].s_code);
	err = get_area_spec(&area_spec,mode,0,e_mesg);
	if(err < 0) {
		DispError((char *)&s_sth,e_mesg);
		return(err);
	}
...
BuildList()
{
	...
	for( ; ; ) {
		retval = get_n_area_spec(&area_spec,BROWSE,0,FORWARD,e_mesg);
		if(retval < 0) {
			if(retval == EFL) break;
			DispError((char *)&s_sth,e_mesg);
			return(retval);
		...
		strcpy(CurPage->Items[i].s_code, area_spec.ar_code);
...
int	mode;
{
	...
	scpy((char *)&pre_area_spec,(char *)&area_spec,sizeof(area_spec));
...
int	mode;
{
	...
	strcpy(area_spec.ar_code,temppage->Items[item_no].s_code);
	if(mode != ADD) {
		retval = get_area_spec(&area_spec,UPDATE,0,e_mesg) ;
		if(retval < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(retval);
	...
	strcpy(area_spec.ar_desc,temppage->Items[item_no].s_desc);

	retval = put_area_spec(&area_spec,mode,e_mesg) ;
	if(retval < 0) {
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(retval);
	...
	if(mode != ADD) {
		retval = rite_audit((char*)&s_sth,AREA_SPEC,mode,(char*)&area_spec,
			(char*)&pre_area_spec,e_mesg);
		if(retval==LOCKED) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg) ;
			return(LOCKED) ;
...
Validation()
{
	...
	case CODE_FLD :			/* Area of Specialization code field */
  		Right_Justify_Numeric(s_sth.s_items[item_no].s_code,
			sizeof(s_sth.s_items[item_no].s_code)-1);
		if(CheckCode(s_sth.s_items[item_no].s_code)<0){
...
WindowHelp()
{
	...
	switch(sr.curfld){
	case CODE_FLD :
		retval = area_spec_hlp( s_sth.s_items[item_no].s_code,
...
#ifdef ENGLISH
	sprintf(e_mesg,	"Area of Specialization Code Already Exists, Please Re-enter");
#else
	sprintf(e_mesg, "Code de area  existe deja, SVP reseille");
#endif
	DispError((char *)&s_sth,e_mesg);
	return(ERROR);
}
