...
#define FUND_FLD	500	/* Fn: */
#define TYPE_FLD	600	/* Fn: */
#define CLASS_FLD	700	/* Fn: */
#define EARN_FLD	800	/* Fn: */
#define CHG_FLD		900
#define PAGE_FLD	1100	/* Desc. Field */

#define CC_FLD		1200	/* Description field */
#define ACCT_FLD	1300	/* Description field */
#define STATUS_FLD	1400	/* Status Field */

#define	ITEM_ST_FLD	1200	/* Item Start Field */
#define ITEM_END_FLD	5600	/* Item End Field */
#define	STEP		300	/* # of Fields on one line */

#define	END_FLD		5800	/* Last Field of the screen */

/* area.sth - header for C structure generated by PROFOM EDITOR */

typedef	struct	{	/* Start Fld 1500, Endfld 4200 */

	short	s_cc;		/* 800 STRING X(6) */
	char	s_acct[19];	/* 900 STRING X(30) */
	char	s_status[4];	/* 1300 STRING XXX */
	char	s_dummy[3];	/* 1300 STRING XXX */
...
static	long 	effective;  		/* dbh will return err msg in this */
static	int 	currentpage;  		/* dbh will return err msg in this */
...
static	Gl_rec	gl_rec;		/* Gl Master file */
...
InitScreen()
{
	...
	s_sth.s_class[0] = HV_CHAR;
	s_sth.s_earn[0] = HV_CHAR;
...
ReadFunction()
{
	/* Display Fn: options */
#ifdef ENGLISH
	fomer("A(dd), C(hange), D(elete), I(nquire), N(ext), P(revious), E(xit)");
#else
...
int	direction ;
{
	...
	err = get_n_glacct(&gl_acct,BROWSE,1,direction,e_mesg);
	if(err < 0 && err != EFL) {
		DispError((char *)&s_sth,e_mesg);
		return(err);
	}
	...
	if(err == EFL) {
		...
		s_sth.s_fund = gl_acct.gl_fund;
		strcpy(s_sth.s_type,gl_acct.gl_type);
		strcpy(s_sth.s_class,gl_acct.gl_class);
		strcpy(s_sth.s_earn,gl_acct.gl_earn);
		ret(WriteFields((char *)&s_sth,FUND_FLD,EARN_FLD));
		...
		gl_acct.gl_fund = s_sth.s_fund;
		strcpy(gl_acct.gl_type,s_sth.s_type);
		strcpy(gl_acct.gl_class,s_sth.s_class);
		strcpy(gl_acct.gl_earn,s_sth.s_earn);
...
{
	int	i;
	char	tmp_class[7],tmp_earn[7],tmp_type[2];
	short	tmp_fund;
	
	/* In Add mode turn off dup control for key fields.
	   Other modes reverse it */
	if(s_sth.s_fn[0] == ADDREC){	/* ADD */
		SetDupBuffers(FUND_FLD,EARN_FLD,0); /* Off Dup Control */
	}
	else {
		SetDupBuffers(FUND_FLD,EARN_FLD,1);
	}
	...
	tmp_fund = s_sth.s_fund;
	strcpy(tmp_type,s_sth.s_type);
	strcpy(tmp_class,s_sth.s_class);
	strcpy(tmp_earn,s_sth.s_earn);

	s_sth.s_fund = LV_SHORT;
	s_sth.s_type[0] = LV_CHAR;
	s_sth.s_class[0] = LV_CHAR;
	s_sth.s_earn[0] = LV_CHAR;

	i = ReadFields((char *)&s_sth,FUND_FLD, EARN_FLD,
		Validation, WindowHelp,1) ;
	if(PROFOM_ERR == i || DBH_ERR == i) return(i) ;
	if(RET_USER_ESC == i){
		
		s_sth.s_fund = tmp_fund;
		strcpy(s_sth.s_type,tmp_type);
		strcpy(s_sth.s_class,tmp_class);
		strcpy(s_sth.s_earn,tmp_earn);
...
AddItems()
{
	...
	for( ; ; ) {
		if( PAGESIZE == idex) {	/* Page Full */
			...
			for(idex--;idex > 0; idex--)
				InitItem(HV_CHAR,HV_SHORT,idex);
...
BuildList()
{
	...
	for( ; ; ) {
		...
		if(gl_acct.gl_fund != s_sth.s_fund || 
		  (strcmp(gl_acct.gl_type,s_sth.s_type) != 0) || 
		  (strcmp(gl_acct.gl_earn,s_sth.s_earn) != 0) || 
		  (strcmp(gl_acct.gl_class,s_sth.s_class) != 0))
			break;
	...
	if( FirstPage==NULL ){
		/* if no items to display, add items from scratch  */
#ifdef ENGLISH
		fomen(INVITEM);
#else
		fomen(INVITEM);
...
Page	*pageptr ;
{
	...
	ret( WriteFields((char *)&s_sth, TYPE_FLD, (END_FLD - 200)) );
...
{
	if(mode == ADD) {
		if(strcmp(temppage->Items[item_no].s_status,ERASED)==0) {
			*write_mode = NOOP;
		}
		else {
			*write_mode = ADD;
...
int	mode;
{
	...
	strcpy(gl_acct.gl_class,s_sth.s_class);
	strcpy(gl_acct.gl_earn,s_sth.s_earn);
	gl_acct.gl_cc = temppage->Items[item_no].s_cc;
	...
	strcpy(gl_acct.gl_acct,temppage->Items[item_no].s_acct);
...
Validation()
{
	...
	case TYPE_FLD :			/* Position code field */
		  if((strcmp(s_sth.s_type,"S") != 0) &&
		     (strcmp(s_sth.s_type,"T") != 0) &&
		     (strcmp(s_sth.s_type,"C") != 0) &&
		     (strcmp(s_sth.s_type,"U") != 0) &&
		     (strcmp(s_sth.s_type,"R") != 0) &&
		     (strcmp(s_sth.s_type,"D") != 0)){ 	
	...
	case EARN_FLD :			/* Position code field */
		...
		if(s_sth.s_type[0] == 'S' ) {
			strcpy(earn.ea_code,s_sth.s_earn);
			...
			if(strcmp(earn.ea_code,s_sth.s_earn) != 0){
				fomer(NOKEY);
				s_sth.s_earn[0] = LV_CHAR;
				return(ERROR);
			}
		  ...
		  if(s_sth.s_type[0] == 'R') {
			strcpy(reg_pen.rg_code,s_sth.s_earn);
			reg_pen.rg_pp_code[0] = '\0';
			flg_reset(REG_PEN);

			retval = get_n_reg_pen(&reg_pen,
			...
			if(strcmp(reg_pen.rg_code,s_sth.s_earn)!=0) {
				fomer(NOKEY);
				s_sth.s_earn[0] = LV_CHAR;
				return(ERROR);
			}
			...
			if(strcmp(s_sth.s_earn,"UIC") == 0 ||
			   strcmp(s_sth.s_earn,"CPP") == 0 ||
			   strcmp(s_sth.s_earn,"TAX") == 0) 
				return(NOERROR);

			strcpy(deduction.dd_code,s_sth.s_earn);
			...
			if(strcmp(deduction.dd_code,s_sth.s_earn)!=0) {
				fomer(NOKEY);
				s_sth.s_earn[0] = LV_CHAR;
				return(ERROR);
			}
	...
	case CLASS_FLD :			/* Position code field */
		Right_Justify_Numeric(s_sth.s_class,sizeof(s_sth.s_class)-1);
		strcpy(class.c_code,s_sth.s_class);
		...
		if(strcmp(class.c_code,s_sth.s_class)!=0) {
			fomer(NOKEY);
			s_sth.s_class[0] = LV_CHAR;
			return(ERROR);
		}
	...
	case CC_FLD :			/* Position code field */
		...
		strcpy(gl_acct.gl_class,s_sth.s_class);
		strcpy(gl_acct.gl_earn,s_sth.s_earn);
		gl_acct.gl_cc = s_sth.s_items[item_no].s_cc;

		retval = get_glacct(&gl_acct,UPDATE,0,e_mesg) ;
		if(retval != UNDEF && s_sth.s_fn[0] != ADDREC) {
			fomer(DUPREC);
			s_sth.s_items[item_no].s_cc = LV_SHORT;
			return(retval);
		}
		...
		if(CheckCode(s_sth.s_items[item_no].s_cc,item_no)<0) {
			fomen(BEENENT);
			s_sth.s_items[item_no].s_cc = LV_SHORT;
			return(ERROR);
		}
	...
	case ACCT_FLD :			/* Position code field */
		...
		STRCPY(gl_rec.accno, s_sth.s_items[item_no].s_acct);
		gl_rec.reccod = 99;
...
WindowHelp()
{
	...
	case	CLASS_FLD:	
		retval = class_hlp(s_sth.s_class,&effective,7,13);
	...
	case	EARN_FLD:	
		if(s_sth.s_type[0] == 'S') {
			retval = earn_hlp(&effective,s_sth.s_earn,7,13);
			if(retval == DBH_ERR) return(retval);
			if(retval >= 0) redraw();
			if(retval == 0) return(ERROR);
			if(retval < 0) return(ERROR);
			...
			earn.ea_code[0] = '\0';
			flg_reset(EARN);
			retval = get_n_earn(&earn,BROWSE,1,FORWARD,e_mesg);
			if(retval < 0) {
				fomer(e_mesg);
				s_sth.s_earn[0] = LV_CHAR;
				return(ERROR);
		  ...
		  if(s_sth.s_type[0] == 'R') {
			retval = reg_pen_hlp(s_sth.s_earn,temp_pp,7,13);
			if(retval == DBH_ERR) return(retval);
			if(retval >= 0) redraw();
			if(retval == 0) return(ERROR);
			if(retval < 0) return(ERROR);
			...
			retval = deduction_hlp(s_sth.s_earn,
				deduction.dd_pp_code,7,13);
			if(retval == DBH_ERR) return(retval);
			if(retval >= 0) redraw();
			if(retval == 0) return(ERROR);
			if(retval < 0) return(ERROR);
...
int	item_no;
{
	...
	for(j=1; ; j++){
		...
		for(idex=0; idex < PAGESIZE; idex++){
			...
			if(currentpage == j && (item_no) == idex){
				continue;
			}

			if(costcenter == LV_SHORT )
				return(NOERROR);

			if(temppage->Items[idex].s_cc == HV_SHORT ||
			    temppage->Items[idex].s_cc == 0)
				return(NOERROR);

			if(temppage->Items[idex].s_cc == costcenter){
				return(-1);
			}
		}
...
ScreenEdit()
{
	...
	for(i=0;i < PAGESIZE; i++) {
		if(s_sth.s_items[i].s_cc == HV_SHORT)
			break;
...
ChangeFields()
{
	...
	DispMesgFld((char *)&s_sth); ;
		...
		if(strcmp(CurPage->Items[s_sth.s_field-1].s_status,ERASED)==0){
			fomer(STATDEL);
			continue;
		}
...
int	status;
{
	...
	DispMesgFld((char *)&s_sth); ;
		...
		st_fld  = ITEM_ST_FLD + (STEP * (s_sth.s_field-1)) + (STEP-100);
		end_fld = ITEM_ST_FLD + (STEP * (s_sth.s_field-1)) + (STEP-100);
...
short	item_no ;
{
	...
	s_sth.s_items[item_no].s_dummy[0] = HV_CHAR;
