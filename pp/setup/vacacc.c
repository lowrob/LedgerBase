/*-----------------------------------------------------------------------
Source name: vacacc.c
System     : Personnel/Payroll System.
Created  On: 93-MAR-23.
Created  By: m. galvin 

DESCRIPTION:
	Program To input the Vacation Tables.
...
#define MAINFL		VC_ACC		/* main file used */

#define	SYSTEM		"SETUP"			/* Sub System Name */
#define	MOD_DATE	"93-MAR-23"		/* Progran Last Modified */
...
#define	SCR_NAME	"vacacc"	/* PROFOM screen Name */
...
#define	END_FLD		3100	/* Last Field of the screen */
...
#define LOW_SEN_FLD	100
#define VAC_FLD		200
#define HIGH_SEN_FLD	300
...
typedef	struct	{	/* Start Fld 900, Endfld 5300 and Step 300 */
	...
	double	s_low_sen;	/* 1000 NUMERIC 9,999,999.99 */
	double	s_vacdays;		/* 1100 NUMERIC 99.999 */
	double	s_high_sen;	/* 1200 NUMERIC 9,999,999.99 */
...
typedef struct	{
	...
	char	s_barg[7];		/* 600 DATE YYYYFMMFDD */
...
static	Vc_acc	vac, pre_vac;	/* Vacation record */
...
int	direction ;
{
	...
	strcpy(vac.vc_barg,s_sth.s_barg);
	vac.vc_low_sen = 0;
	vac.vc_high_sen = 0;
	if(flg_start(VC_ACC) != direction) {
		if (direction == FORWARD) 
			vac.vc_low_sen += 0.0001;
		else
			vac.vc_low_sen -= 0.0001;
		flg_reset(VC_ACC);
	}

	err = get_n_vc_acc(&vac,BROWSE,0,direction,e_mesg);
	if(err < 0 && err != EFL) {
		DispError((char *)&s_sth,e_mesg);
		return(err);
	}
	...
	if(err == EFL) {
		...
		strcpy(s_sth.s_barg,vac.vc_barg);
...
AddItems()
{
	...
	for( ; ; ) {
		...
		if(s_sth.s_items[idex-1].s_high_sen == 999999.99) {
			break;
		}
	}
	if(idex == 0) 
...
BuildList()
{
	...
	for( ; ; ) {
		...
		if(strcmp(vac.vc_barg,s_sth.s_barg)!=0)
			break;
		...
		CurPage->Items[idex].s_low_sen = vac.vc_low_sen;
		CurPage->Items[idex].s_high_sen = vac.vc_high_sen;
		CurPage->Items[idex].s_vacdays = vac.vc_days;
...
int	mode;
{
	...
	strcpy(vac.vc_barg,s_sth.s_barg);
	vac.vc_low_sen = temppage->Items[item_no].s_low_sen ;
	vac.vc_high_sen = temppage->Items[item_no].s_high_sen ;
	vac.vc_days = temppage->Items[item_no].s_vacdays ;
	if(mode != ADD) {
		scpy((char *)&pre_vac,(char *)&vac, sizeof(pre_vac));

		retval = get_vc_acc(&vac,UPDATE,0,e_mesg) ;
		if(retval < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(retval);
	...
	retval = put_vc_acc(&vac,mode,e_mesg) ;
	if(retval < 0) {
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(retval);
	...
	if(mode != ADD) {
		retval = rite_audit((char*)&s_sth,VC_ACC,mode,(char*)&vac,
			(char*)&pre_vac,e_mesg);
		if(retval==LOCKED) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg) ;
...
int	mode ;
{
	...
	if(mode == ADD) {
		...
		s_sth.s_items[item_no].s_vacdays = LV_DOUBLE;
	}
...
Validation()
{
	...
	if(sr.curfld >= ITEM_ST_FLD) {	
		...
		case	HIGH_SEN_FLD:
			if(s_sth.s_items[item_no].s_high_sen <= 
			   s_sth.s_items[item_no].s_low_sen){
				s_sth.s_items[item_no].s_high_sen = LV_DOUBLE;
		...
		case	KEY_START:
			...
			strcpy(barg_unit.b_code,s_sth.s_barg);
			barg_unit.b_date = get_date();
			flg_reset(BARG);

			retval=get_n_barg(&barg_unit,BROWSE,0,BACKWARD,e_mesg);
			if(retval == EFL ||
				strcmp(barg_unit.b_code, s_sth.s_barg) != 0){
  				DispError((char *)&s_sth,
					"Bargaining Unit does not Exist");
			  	s_sth.s_barg[0] = LV_CHAR;
				return(ERROR);
			}
			seq_over(BARG);
			...
			if(s_sth.s_fn[0] == ADDREC) {
				if((strcmp(s_sth.s_barg,vac.vc_barg)==0) && retval !=EFL){
					fomen("Bargaining Unit's Vacation File Already Exists");
					s_sth.s_barg[0] = LV_CHAR;
					return(-1);
				}
			}
				...
				if((strcmp(s_sth.s_barg,vac.vc_barg)!=0) || retval==EFL){
					fomen("Bargaining Unit's Vacation File Does Not Exist");
					s_sth.s_barg[0] = LV_CHAR;
					return(-1);
...
char 	t_char ;
{
	...
	if(s_sth.s_fn[0] != CHANGE) {
		s_sth.s_items[item_no].s_low_sen  = t_double;
		s_sth.s_items[item_no].s_high_sen = t_double ;
		s_sth.s_items[item_no].s_status[0] = t_char ;
	}
	s_sth.s_items[item_no].s_vacdays = t_double ;
...
char 	t_char ;
{
	...
	if(item_no == 0 && s_sth.s_page == 1 )  {
		s_sth.s_items[item_no].s_low_sen = LV_DOUBLE;
	}
	else{
		if(item_no == 0)  {
			s_sth.s_items[item_no].s_low_sen = CurPage->
				Items[PAGESIZE-1].s_high_sen + 0.01;
		}
		else {
			s_sth.s_items[item_no].s_low_sen = 
				s_sth.s_items[item_no-1].s_high_sen + 0.01;
		}
	}
...
FreeList()	
{
	...
	for(CurPage = FirstPage;CurPage;CurPage = CurPage->NextPage){
		...
		for( idex=0; idex <= PAGESIZE; idex++) {
			...
			CurPage->Items[idex].s_low_sen = HV_SHORT;
		}
	}
