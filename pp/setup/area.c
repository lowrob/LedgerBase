...
DESCRIPTION:
	Program To input the different areas found within each department.
...
#define	SCR_NAME	"area"	/* PROFOM screen Name */
...
typedef	struct	{	/* Start Fld 900, Endfld 5300 and Step 300 */
	...
	char	s_area_cd[7];	/* 900 STRING X(6) */
	char	s_area_desc[32];/* 1000 STRING X(31) */
	char	s_status[5]; 	/* 1100 STRING X(4) */
}	S_item ;
...
static	Dept	dept_rec;		/* Department record */
static	Area	area_rec, pre_area;	/* Area record */
...
int	direction ;
{
	...
	strcpy(area_rec.a_deptcode,s_sth.s_dept);
	inc_str(area_rec.a_deptcode,sizeof(area_rec.a_deptcode)-1,direction);
	if(direction == BACKWARD) {
		strcpy(area_rec.a_code,"ZZZZZZ") ;
	}
	else {
		area_rec.a_code[0] = '\0' ;
	}
	...
	err = get_n_area(&area_rec,BROWSE,0,direction,e_mesg);
	if(err < 0 && err != EFL) {
		DispError((char *)&s_sth,e_mesg);
		return(err);
	}
	...
	if(err == EFL) {
		...
		strcpy(s_sth.s_dept,area_rec.a_deptcode);
...
BuildList()
{
	...
	for( ; ; ) {
		...
		if(strcmp(area_rec.a_deptcode,s_sth.s_dept)!=0) { 
			break;
		}
		...
		strcpy(CurPage->Items[i].s_area_cd,area_rec.a_code);
		strcpy(CurPage->Items[i].s_area_desc,area_rec.a_desc);
...
int	mode;
{
	...
	if(mode != ADD) {
		scpy((char *)&pre_area,(char *)&area_rec,
			sizeof(pre_area));

		strcpy(area_rec.a_deptcode,s_sth.s_dept) ;
		strcpy(area_rec.a_code,temppage->Items[item_no].s_area_cd);
		retval = get_area(&area_rec,UPDATE,0,e_mesg) ;
		if(retval < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(retval);
	...
	strcpy(area_rec.a_deptcode,s_sth.s_dept);
	strcpy(area_rec.a_code,temppage->Items[item_no].s_area_cd);
	strcpy(area_rec.a_desc,temppage->Items[item_no].s_area_desc);

	retval = put_area(&area_rec,mode,e_mesg) ;
	if(retval < 0) {
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(retval);
	...
	if(mode != ADD) {
		retval = rite_audit((char*)&s_sth,AREA,mode,(char*)&area_rec,
...
int	mode ;
{
	...
	if(mode == ADD) {
		st_fld  = ITEM_ST_FLD + (STEP * item_no) + AREA_FLD;
		end_fld  = ITEM_ST_FLD + (STEP * item_no) + STATUS_FLD;
		s_sth.s_items[item_no].s_area_cd[0] = LV_CHAR;
		s_sth.s_items[item_no].s_area_desc[0] = LV_CHAR;
...
Validation()
{
	...
	if(sr.curfld == KEY_START) {
		...
		item_no = (sr.curfld - ITEM_ST_FLD - AREA_FLD) / STEP;
	...
	case KEY_START:
		if(s_sth.s_dept[0] == '\0') {
  		...
  		Right_Justify_Numeric(s_sth.s_dept,sizeof(s_sth.s_dept)-1);
		ret(WriteFields((char *)&s_sth,KEY_START,KEY_END));

		strcpy(dept_rec.d_code,s_sth.s_dept);
		retval = get_dept(&dept_rec,BROWSE,0,e_mesg);
		if(retval < 0 ){
			fomer(e_mesg);
			s_sth.s_dept[0] = LV_CHAR;
			return(ERROR);
		}
		break;
	case ITEM_AREA:
  		Right_Justify_Numeric(s_sth.s_items[item_no].s_area_cd,
			sizeof(s_sth.s_items[item_no].s_area_cd)-1);
		if(CheckAreaCode(s_sth.s_items[item_no].s_area_cd)<0) {
			s_sth.s_items[item_no].s_area_cd[0] = LV_CHAR;
			return(ERROR);
		}
		break;
	case ITEM_DESC:
		if(s_sth.s_items[item_no].s_area_desc[0] == '\0') {
			fomer("This is a Required Field");
			return(ERROR);
		}
		break;
...
char	*area_cd;
{
	...
	strcpy(area_rec.a_deptcode,s_sth.s_dept);
	strcpy(area_rec.a_code,area_cd);
	...
	if(retval < 0) {
		...
		fomer("Area Code Already Exists on File");
		return(ERROR);
	}
	...
	if(CurLast != NULL) {
	   for(temppage=FirstPage; temppage!=NULL;temppage=temppage->NextPage) {
	      for(i =0; i< temppage->NoItems; i++) {
			if(temppage->Items[i].s_area_cd[0] == NULL) {
				return(NOERROR);
			}
			if(strcmp(temppage->Items[i].s_area_cd,area_cd) == 0) {
				fomer("Area Code Already Entered on Screen");
				return(ERROR);
			}
	      }
	      if(temppage == CurLast) break;
...
WindowHelp()
{
	...
	switch(fld_no){
	case KEY_START:
		retval = dept_hlp(s_sth.s_dept,7, 13);
		...
		break;
	case ITEM_AREA:
		retval = area_hlp(s_sth.s_dept,
			s_sth.s_items[item_no].s_area_cd,7,13);
		if(retval == DBH_ERR) return(retval);
		if(retval >= 0) redraw();
		if(retval == 0) return(ERROR);
		if(retval < 0) return(ERROR);
...
char	t_char ;
{
	...
	s_sth.s_items[item_no].s_area_cd[0] = t_char ;
	s_sth.s_items[item_no].s_area_desc[0] = t_char ;
