...
Created  On: 8th June. 93.
Created  By: Andre Cormier    

DESCRIPTION:
	The Competition Code Maintenance Screen is used to identify the 
	different competition codes.  
...
#define MAINFL		COMP			/* main file used */

#define	SYSTEM		"SETUP"			/* Sub System Name */
#define	MOD_DATE	"08-JUNE-93"		/* Progran Last Modified */
...
#define	SCR_NAME	"comp"	/* PROFOM screen Name */
...
#define HDR_END_FLD	1000	/* Units/Year Field */

#define	CODE_FLD	800	/* CSB/Loan Code */
#define DESC_FLD	900	/* Description field */
#define INT_ACCT	1000	/* Interest G/L Account */

#define STATUS_FLD	200	/* Status Field */

#define PAGE_FLD	700	/* Page# Field */

#define	ITEM_ST_FLD	800	/* Item Start Field */
#define ITEM_END_FLD	5200	/* Item End Field */
...
typedef	struct	{	/* Start Fld 1500, Endfld 4200 */
	...
	char	s_code[8];	/* 800 STRING X(6) */
	char	s_desc[41];	/* 900 STRING X(30) */
...
#else
	fomer("R(ajouter), C(hanger), E(liminer), I(nterroger), F(in)");
#endif
...
int	mode;
{
	...
	strcpy(comp.cm_code,s_sth.s_items[0].s_code);
	err = get_comp(&comp,mode,0,e_mesg);
	if(err < 0) {
		DispError((char *)&s_sth,e_mesg);
		return(err);
	}
...
BuildList()
{
	...
	for( ; ; ) {
		...
		strcpy(CurPage->Items[i].s_code, comp.cm_code);
...
int	mode;
{
	...
	scpy((char *)&pre_comp,(char *)&comp,sizeof(comp));
...
int	mode;
{
	...
	strcpy(comp.cm_code,temppage->Items[item_no].s_code);
	if(mode != ADD) {
		retval = get_comp(&comp,UPDATE,0,e_mesg) ;
		if(retval < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(retval);
	...
	strcpy(comp.cm_desc,temppage->Items[item_no].s_desc);

	retval = put_comp(&comp,mode,e_mesg) ;
	if(retval < 0) {
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(retval);
	...
	if(mode != ADD) {
		retval = rite_audit((char*)&s_sth,COMP,mode,(char*)&comp,
			(char*)&pre_comp,e_mesg);
		if(retval==LOCKED) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg) ;
			return(LOCKED) ;
...
int	mode ;
{
	...
	if(mode == ADD) {
		InitItem(LV_CHAR,LV_LONG,item_no);
	}
	else {
	...
	if(RET_USER_ESC == i) {
		if(mode == ADD) {
			InitItem(HV_CHAR,HV_LONG,item_no);
...
Validation()
{
	...
	case CODE_FLD: 
		...
		if(s_sth.s_fn[0] == ADDREC) {
			...
			if(retval == NOERROR) {
				fomer("Competition Code Already Exists");
				s_sth.s_items[item_no].s_code[0] = LV_CHAR;
				return(ERROR);
			}
		}
		retval = CheckCode(s_sth.s_items[item_no].s_code);
		if(retval < 0) {
			s_sth.s_items[item_no].s_code[0] = LV_CHAR;
			return(ERROR);
		}
...
InitItem(t_char, t_long, item_no)
char	t_char ;
long	t_long ;
short	item_no ;
{
