...
m. galvin      92/08/08	      Modified the program to check
			      to see if the termination code
			      is present on the employee file
			      before allowing the operator
			      to delete it.
------------------------------------------------------------------------*/
#define	MAIN		/* Main program. This is to declare Switches */
#define MAINFL		TERM		/*main file used */
...
#define	SCR_NAME	"termination"	/* PROFOM screen Name */
...
static	Term	termination,	 pre_termination;
static	Emp	emp_rec;
...
int	mode;
{
	...
	strcpy(termination.t_code,s_sth.s_items[0].s_code);
	err = get_pterm(&termination,mode,0,e_mesg);
	if(err < 0) {
		DispError((char *)&s_sth,e_mesg);
		return(err);
	}
...
BuildList()
{
	...
	termination.t_code[0] = '\0';	
	flg_reset(TERM);

	for( ; ; ) {
		retval = get_n_pterm(&termination,BROWSE,0,FORWARD,e_mesg);
		if(retval < 0) {
			if(retval == EFL) break;
			DispError((char *)&s_sth,e_mesg);
			return(retval);
		...
		strcpy(CurPage->Items[i].s_code, termination.t_code);
		strcpy(CurPage->Items[i].s_desc, termination.t_desc);
...
int	mode;
{
	...
	scpy((char *)&pre_termination,(char *)&termination,sizeof(termination));
...
int	mode;
{
	...
	strcpy(termination.t_code,temppage->Items[item_no].s_code);
	if(mode != ADD) {
		retval = get_pterm(&termination,UPDATE,0,e_mesg) ;
	...
	strcpy(termination.t_desc,temppage->Items[item_no].s_desc);

	retval = put_pterm(&termination,mode,e_mesg) ;
	if(retval < 0) {
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(retval);
	...
	if(mode != ADD) {
		retval = rite_audit((char*)&s_sth,TERM,mode,(char*)&termination,
			(char*)&pre_termination,e_mesg);
		if(retval==LOCKED) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg) ;
			return(LOCKED) ;
...
Validation()
{
	...
	case CODE_FLD :			/* Termination code field */
	  strcpy(termination.t_code,s_sth.s_items[item_no].s_code);
	...
	/*   Verify that the termination code is not already present in the
	     linked list.						*/

	  if(CurLast != NULL){
	    strcpy(tempcode, s_sth.s_items[item_no].s_code);
	    ...
	    for(temppage=FirstPage; temppage<=LastPage;
	      ...
	      for(i = 0; i < CurPage->NoItems; i++) {
	        ...
	        if(strcmp(temppage->Items[i].s_code,
...
WindowHelp()
{
	...
	switch(sr.curfld){
	case CODE_FLD :
		retval = term_hlp( s_sth.s_items[item_no].s_code,
		...
		if(status == DELITEM) {
			if(strcmp(s_sth.s_items[s_sth.s_field-1].s_status,
			   INACTIVE)==0) {
				...
				for(;;) {
				  get_n_employee(&emp_rec,BROWSE,0,FORWARD,e_mesg);
				  ...
				  if(strcmp(emp_rec.em_term,
				     s_sth.s_items[s_sth.s_field-1].s_code)!=0){
					continue;
				  }
				  else {
...
#ifdef ENGLISH
	sprintf(e_mesg,	"Termination Code Already Exists, Please Re-enter");
#else
	sprintf(e_mesg, "Code de termination existe deja, SVP reseille");
#endif
	DispError((char *)&s_sth,e_mesg);
	return(ERROR);
}
