...
m. galvin	1992/08/24	  I modifed the program due to the changes
				  made to the files that are read.  I also
				  added a description to print at the top
...
PrintRpt()
{
	char	old_code[3], last_barg[7], last_pos[7], last_class[7];
	short	last_center;
	int	first_time = 0;
	...
	*/
	...
	if(sortop2[0]== 'C'||sortop2[0]=='L'){
		...
		strcpy(old_code,emp_rec.em_term);
		...
		for(;;){
			...
			/* Get the termination description */
			strcpy(term_rec.t_code,emp_rec.em_term);
			flg_reset(TERM);
...
PrntHdg()	/* Print heading  */
{
...
#ifdef ENGLISH
	mkln((LNSZ-37)/2,"LIST OF EMPLOYEES BY TERMINATION CODE", 37 );
#else
	...
	mkln(1,"TERMINATION",11);
	mkln(28,"REASON",6);
	mkln(48,"EMPLOYEE",8);
	mkln(68,"EMPLOYEE NAME",13);
	mkln(94,"DATE OF",7);
	if(prnt_line() < 0 )	return(REPORT_ERR);
	mkln(4,"CODE",4);
	mkln(49,"NUMBER",6);
	mkln(92,"TERMINATION",11);
...
PrntRec()
{
	...
	if(strcmp(old_code,emp_rec.em_term)!=0){
		strcpy(txt_line,"        ");
		mkln(4,emp_rec.em_term,2);
		mkln(14,term_rec.t_desc,30);
	}
	mkln(46,emp_rec.em_numb,12);
	sprintf(txt_line,"%s, %s %s",
		emp_rec.em_last_name,
		emp_rec.em_first_name,
		emp_rec.em_mid_name);
	mkln(60,txt_line,30);
	sprintf(txt_line,"%ld",emp_rec.em_term_dt);
	mkln(92,txt_line,4);
	mkln(96,"/",1);
	mkln(97,txt_line+4,2);
	mkln(99,"/",1);
	mkln(100,txt_line+6,2);
	if(prnt_line() < 0 )	return(REPORT_ERR);
...
PutTmpSchedT()
{
	...
	strcpy(tmp_sched1.tm_sortk_2,emp_rec.em_term);
...
char	*tempindxnm;	/*temparary index file name*/
{
	...
	switch (sortop2[0]){
		case 'B':	/*Name within Bargain unit key*/
			...
			keysarray[step++] = CHAR;
			keysarray[step++] =  2;
			keysarray[step++] = (char*)&emp_rec.em_term[0] - (char*)&emp_rec;
			keysarray[step++] = ASCND;
