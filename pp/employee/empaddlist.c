...
m. galvin	1992/08/24 	  Modified the program since the layout of the
				  files used in this program have been changed.
				  I also added a few things: a description and
				  code of the sort option selected, modified  
				  the report layout (added extra data to 
				  match the report on the old system) & a check
				  if the employee's status was active.
******************************************************************************/
...
empaddlist()
{
	...
	switch(*resp) {
		...
		case FILE_IO:
			resp[0]='F';
			strcpy( discfile, "empaddlist.dat" );
	...
	*/
	if(sortop2[0]=='C'||sortop2[0]=='L'){
		...
		for(;;){
			...
			teacher = 0;
			/* SJ class codes 
			if(strcmp(tmp_sched1.tm_class,"  0101") == 0) 
				teacher = 1;
			if(strcmp(tmp_sched1.tm_class," 01012") == 0)
				teacher = 1;
			if(strcmp(tmp_sched1.tm_class," 01013") == 0)
				teacher = 1;
			if(strcmp(tmp_sched1.tm_class," 01014") == 0)
				teacher = 1;
			if(strcmp(tmp_sched1.tm_class," 01015") == 0) 
				teacher = 1;
			if(strcmp(tmp_sched1.tm_class," 01016") == 0) 
				teacher = 1;
			if(strcmp(tmp_sched1.tm_class," 01017") == 0)
				teacher = 1;
			if(strcmp(tmp_sched1.tm_class," 01018") == 0)
				teacher = 1; */

			/* Fredericton class codes */
			if(strncmp(tmp_sched1.tm_class,"0101",4) == 0)
				teacher = 1;
			...
			if(teacher == 1) {
				...
				retval = get_teach_ass(&teach_ass,BROWSE,0,e_mesg);
				...
				if(retval == UNDEF) {
					...
					strcpy(area_spec.ar_code,
							teach_ass.tc_ar_sp);
					retval = get_area_spec(&area_spec,
							BROWSE,0,e_mesg);
					if(retval < 0) {	
						fomen(e_mesg);
						get();
	...
	if(sortop2[0]=='B'||sortop2[0]=='P'){
		...
		for (;;){
			...
			if(teacher == 1) {
				no_teach_ass = 0;
				strcpy(teach_ass.tc_numb,emp_rec.em_numb);
				teach_ass.tc_cost = emp_rec.em_cc;
				retval = get_teach_ass(&teach_ass,BROWSE,0,e_mesg);
				if(retval < 0 && retval != UNDEF) {
					fomen(e_mesg);
					get();
...
PrntHdg()	/* Print heading  */
{
...
#ifdef ENGLISH
	mkln((LNSZ-17)/2,"LIST OF ADDRESSES", 17 );
#else
	...
	mkln(56,"MAILING/STREET ADDRESS",22);
	mkln(87,"TELEPHONE",9);
	mkln(109,"TITLE",5);
	if(sortop2[0] != 'P') {
		mkln(128,"TYPE",4);
...
PrntRec()
{
	...
	mkln(86,emp_rec.em_phone,3);
	mkln(89,"-",1);
	mkln(90,emp_rec.em_phone+3,3);
	mkln(93,"-",1);
	mkln(94,emp_rec.em_phone+6,4);
	if(teacher == 1) {
		if(no_teach_ass == 0) {
			mkln(99,area_spec.ar_desc,28);
		}
	}
	else {
		mkln(99,class.c_desc,28);
	}
	if(sortop2[0] != 'P') {
		mkln(129,position.p_type,2);	
	}
	if(prnt_line() < 0 )	return(REPORT_ERR);
