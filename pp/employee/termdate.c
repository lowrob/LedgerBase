...
m. galvin	1992/08/24	  Modified the program since the files were 
				  changed.  I also added a description to 
				  appear at the top of each page to show
...
PrintRpt()
{
	...
	*/
	if(sortop2[0]== 'C'){
		...
		last_term_date = emp_rec.em_term_dt;
...
#ifdef ENGLISH
	mkln((LNSZ-37)/2,"LIST OF EMPLOYEES BY TERMINATION DATE", 37 );
#else
...
PrntRec()
{
	...
	if(last_term_date != emp_rec.em_term_dt){
		strcpy(txt_line,"        ");
...
PutTmpTdate()
{
	...
	if(sortop2[0] == 'L'){
 	...
 	sprintf(sort_array,"%ld",emp_rec.em_term_dt);
...
char	*tempindxnm;	/*temparary index file name*/
{
	...
	switch (sortop2[0]){
		case 'B':	/*Name within Bargain unit key*/
			...
			keysarray[step++] = LONG;
			keysarray[step++] =  1;
			keysarray[step++] = (char*)&emp_rec.em_term_dt - (char*)&emp_rec;
			keysarray[step++] = ASCND;
