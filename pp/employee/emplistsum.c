/******************************************************************************
		Sourcename   : emplistsum.c
		System       : 
		Module       :
		Created on   : 92-SEP-16 
		Created  By  : m. galvin 
******************************************************************************
About the file:	
	This program will print a summary listing of employees.
...
emplistsum()
{
	...
	switch(*resp) {
		...
		case FILE_IO:
			resp[0]='F';
			STRCPY( discfile, "emplistsum.dat" );
...
PrintRpt()
{
	...
	/* get the cost center name and the teaching codes*/
...
PrntHdg()	/* Print heading  */
{
...
#ifdef ENGLISH
	mkln((LNSZ-24)/2,"EMPLOYEE SUMMARY LISTING", 24 );
#else
	...
	mkln(3,"EMPLOYEE",8);
	mkln(26,"NAME",4);
	mkln(45,"COST CENTER",11);
	mkln(58,"CERT",4);
	mkln(64,"LEVEL",5);
	mkln(72,"TEL NUMBER",10);
	mkln(96,"ADDRESS",7);
	if(prnt_line() < 0 )	return(REPORT_ERR);
...
PrntRec()
{
	...
	sprintf(txt_line,"%s, %s %s",
		...
		emp_rec.em_mid_name);
	mkln(15,txt_line,30);
	tedit( (char *)&tmp_cc,"___0",txt_buff, R_SHORT ); 
	mkln(48,txt_buff,4);
	txt_line[0] = '\0';
	mkln(58,emp_rec.em_cert,6);
	mkln(65,emp_rec.em_level,2);
	mkln(71,emp_rec.em_phone,3);
	mkln(74,"-",1);
	mkln(75,emp_rec.em_phone+3,3);
	mkln(78,"-",1);
	mkln(79,emp_rec.em_phone+6,4);
	if(emp_rec.em_add1[0] != '\0') 
		mkln(90,emp_rec.em_add1,30);	
	else
		mkln(90,emp_rec.em_add2,30);
	if(prnt_line() < 0 )	return(REPORT_ERR);
