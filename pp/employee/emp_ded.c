...
L.Robichaud	96/10/15	Added a flag in the BuildList routine 
			called some_info to stop the attempt to copy info
			to the s_dedcd & s_group fields when they have not
			been created. This was causing core dumps.
------------------------------------------------------------------------*/
...
int	item_no;
{
	...
	if(retval == 0){
		sprintf(e_mesg,
			"Record Already Exists For %s %s %s",
			empded.ed_numb,empded.ed_code,empded.ed_group);
		fomen(e_mesg);
	...
	/** Louis. This is being hit, feel the list is not beeing cleared */
			sprintf(e_mesg,
				"Record Already Exists For %s %s %s",
				empded.ed_numb,empded.ed_code,empded.ed_group);
			fomen(e_mesg);
...
BuildList()
{
	...
	if(some_info == YES)
		if(PAGESIZE != i){
			CurPage->Items[i].s_dedcd[0] = NULL;
			CurPage->Items[i].s_group[0] = NULL;
		}
