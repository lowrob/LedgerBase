...
pensalcont()
{
	...
	pension[0] = ALL;
	fomen(" USE / TO PROCESS ALL PENSION CODES");
	if((retval = GetPension(&pension))<0)
		return(retval);
...
/***
strcpy(emp_rec.em_sin, "116337130");
***/
	flg_reset(EMPLOYEE);
	...
	for(;;) {
		...
		for(;;){
		  ...
		  for(;;) {
			...
			if(strncmp(emp_earn.en_reg_pen,"PSPP",4)!=0)
				continue;

			if(pension[0] != ALL)
				if(strcmp(emp_earn.en_reg_pen, pension)){
/***
printf("\npension %s not equal %s",emp_earn.en_reg_pen, pension);
getchar();
***/
					continue;
			}
/***
printf("\ntrying %s on %s",emp_rec.em_numb,emp_earn.en_numb);get();
***/
...
GetPenBuy()
{
	...
	strcpy(emp_ln_his.elh_code,"PENBY5");
	strcpy(emp_ln_his.elh_numb ,emp_rec.em_numb);
	emp_ln_his.elh_date = earn_date;
...
GetOpt()
{
	...
	strcpy(emp_dh.edh_code,"OPPEN5");
	strcpy(emp_dh.edh_group,"OPPEN5");
	strcpy(emp_dh.edh_numb ,emp_rec.em_numb);
	emp_dh.edh_date = earn_date;
	...
	strcpy(emp_dh.edh_code,"PENBY5");
	strcpy(emp_dh.edh_group,"PENBY5");
	strcpy(emp_dh.edh_numb ,emp_rec.em_numb);
	emp_dh.edh_date = earn_date;
...
PrntRec()
{
	...
	sprintf(e_mesg,"Processing %s",emp_rec.em_numb);
	fomen(e_mesg);
	fflush(stdout);
/***
printf("\nInto PrntRec for %s",emp_rec.em_numb);
***/
...
***/
	...
	sprintf(e_mesg,"Printing to report for %s",emp_rec.em_numb);
	fomen(e_mesg);
	fflush(stdout);
