...
static struct	s_struct	{
	...
	char	s_pension[7];	/* pension code for pensalcont.c by L.R. */
	...
	char	s_ded1[7];	/* 3600 start deduction */
	char	s_ded2[7];	/* 3700 ending deduction */
	char	s_class1[7];	/* 3800 starting area */
	char	s_class2[7];	/* 3900 ending area */
	char	s_pos1[7];	/* 4000 starting position */
	char	s_pos2[7];	/* 4100 ending position */
	long	s_enddate;	/* 4200 start date */
	char	s_emp1[13];	/* 4300 starting employee */
	char	s_emp2[13];	/* 4400 ending employee */
	long	s_date1;	/* 4500 start date */
	long	s_date2;	/* 4600 ending date */
	char	s_mesg[78];	/* 4700 message field */
	char	s_resp[2];	/* 4800 response field */
}	s_rec;
...
short	val1, val2;
{
	s_rec.s_ded1[0] = HL_CHAR( val1 );
	s_rec.s_ded2[0] = HL_CHAR( val2 );
	return(0);
}
static
...
char *pen;
{
	...
	column = 1; /*initialize column at start each time*/
	if((retval=GetPosition(PEN))<0){
		return(retval);
	}

	line[PEN] = multi_line;
	fomca2( PENSION,1, line[PEN], 1 +  (column * 42) );
	fomca2( PENSION,2, line[PEN], 19 + (column * 40) );

	strcpy( s_rec.s_pension, pen );
	fomca1( PENSION, 19, 2 );
	sr.nextfld = PENSION;
	sr.endfld = PENSION;
	fomud( (char *)&s_rec);
	s_rec.s_pension[0] = LV_CHAR;
	retval = ReadFields( PENSION ,PENSION);
	if( retval!=EXIT && retval>=0 ){
		Right_Justify_Numeric(s_rec.s_pension,
					sizeof(s_rec.s_pension)-1);
		strcpy( pen, s_rec.s_pension );
	}
	fomca1( PENSION, 19, 0 );

	return(retval);
}
GetDedRange( ded1, ded2 )
char *ded1, *ded2;
{
	int	retval=0;

	column = 1; /*initialize column at start each time*/
	if((retval=GetPosition(DED))<0){
		return(retval);
	}

	line[DED] = multi_line;
	fomca2( DED1,1, line[DED], 1 +  (column * 42) );
	fomca2( DED1,2, line[DED], 19 + (column * 40) );
	fomca2( DED2,1, line[DED], 28 + (column * 40) );
	fomca2( DED2,2, line[DED], 32 + (column * 40) );

	strcpy( s_rec.s_ded1, ded1 );
	strcpy( s_rec.s_ded2, ded2 );
	fomca1( DED1, 19, 2 );
	fomca1( DED2, 19, 2 );
	sr.nextfld = DED1;
	sr.endfld = DED2;
	fomud( (char *)&s_rec);
	s_rec.s_ded1[0] = LV_CHAR;
	s_rec.s_ded2[0] = LV_CHAR;
	retval = ReadFields( DED1, DED2 );
	if( retval!=EXIT && retval>=0 ){
		Right_Justify_Numeric(s_rec.s_ded1,
					sizeof(s_rec.s_ded1)-1);
		Right_Justify_Numeric(s_rec.s_ded2,
					sizeof(s_rec.s_ded2)-1);
		strcpy( ded1, s_rec.s_ded1 );
		strcpy( ded2, s_rec.s_ded2 );
	}
	fomca1( DED1, 19, 0 );
	fomca1( DED2, 19, 0 );
...
short *center1, *center2;
{
	...
	fomca2( CENTER1,1, line[CENTER], 1 + (40 * column));
	fomca2( CENTER1,2, line[CENTER], 19 + (40 * column ));
	fomca2( CENTER2,1, line[CENTER], 30 + (39 * column) );
	fomca2( CENTER2,2, line[CENTER], 34 + (39 * column) );
...
char *class1, *class2;
{
	...
	fomca2( CLASS2,1, line[CLASS], 30 + (39 * column));
	fomca2( CLASS2,2, line[CLASS], 34 + (39 * column));
...
long *date1, *date2;
{
	...
	fomca2( DATE1,2, line[DAT], 10 + (40 * column ));
	fomca2( DATE2,1, line[DAT], 22 + (40 * column) );
	fomca2( DATE2,2, line[DAT], 28 + (40 * column) );
...
GetEndDateRange( enddate )
long *enddate;
{
	int	retval=0;

	if(retval=GetPosition(ENDDAT)<0){ 
		return(retval);
	}
	line[ENDDAT] = multi_line;
	fomca2( ENDDAT1,1, line[ENDDAT], 1 + (40 * column));
	fomca2( ENDDAT1,2, line[ENDDAT], 23 + (40 * column ));

	s_rec.s_enddate = *enddate;
	fomca1( ENDDAT1, 19, 2 );
	sr.nextfld = ENDDAT1;
	sr.endfld = ENDDAT1;
	fomud( (char *)&s_rec);
	s_rec.s_enddate = LV_LONG; 
	retval = ReadFields( ENDDAT1, ENDDAT1 );
	if( retval!=EXIT && retval>=0 ){
		*enddate = s_rec.s_enddate; 
	}
	fomca1( ENDDAT1, 19, 0 );
...
Validate()	/* Validate the values entered by the user */
{
	...
	switch( sr.curfld ){
		...
		case SORTOP2:	/* sort option one */
...
#ifdef ENGLISH
				fomer ("(B)arg, (L)ocation, (C)lass, (P)osition");
#else
				fomer ("TRANSLATE");
#endif
			...
			break;
		case ENDDAT1:      /* starting Bargaining unit */
			julian = days(s_rec.s_enddate);
			remain = julian%7;
			if(remain != 2) {
				fomer("Not a Period Ending Date");
				s_rec.s_enddate = LV_LONG;
			}
			break;
		...
		case BARG2:      /* ending Bargaining unit */
			...
			break;
		case PENSION:	/* Pension Code */
			Right_Justify_Numeric(s_rec.s_pension,
				sizeof(s_rec.s_pension)-1);
			break;

		case DED1:      /* starting Deduction code */
			Right_Justify_Numeric(s_rec.s_ded1,
						sizeof(s_rec.s_ded1)-1);
			break;
		case DED2:      /* ending Deduction code */
			Right_Justify_Numeric(s_rec.s_ded2,
						sizeof(s_rec.s_ded2)-1);
			if(strcmp(s_rec.s_ded2,s_rec.s_ded1 ) <0){
#ifdef ENGLISH
				fomer("Ending Deduction Code can't be less than starting Deduction Code");
#else
				fomer("TRANSLATE");
#endif
				s_rec.s_ded2[0] = LV_CHAR;
...
#ifdef ENGLISH
				fomer("Ending Classification Code  can't be less than starting Classification Code");
#else
				fomer("TRANSLATE");
#endif
				s_rec.s_class2[0] = LV_CHAR;
		...
		case EMP2:      /* ending Bargaining unit */
			Right_Justify_Numeric(s_rec.s_emp2,
						sizeof(s_rec.s_emp2)-1);
			if(strcmp(s_rec.s_emp2,s_rec.s_emp1 ) <0){
#ifdef ENGLISH
...
#ifdef ENGLISH
	fomer("(B)argain, (L)ocation, (C)lass, (P)osition");
#else
	fomer("TRANSLATE");
#endif
	...
	*/
	switch (sortop2[0]){
		...
		case 'P':	/*Name within Position code key*/
			...
			keysarray[step++] = ASCND;
			break;
		case 'C':	/*Name within Position code key*/
			...
			keysarray[step++] = CHAR;
			keysarray[step++] =  6;
			keysarray[step++] = (char*)&emp_rec.em_class[0] - (char*)&emp_rec;
			keysarray[step++] = ASCND;
			...
			keysarray[step++] = ASCND;
			break;
		case 'L':	/*Name within Position code key*/
			...
			keysarray[step++] =  1;
			keysarray[step++] = (char*)&emp_rec.em_cc - (char*)&emp_rec;
			keysarray[step++] = ASCND;
