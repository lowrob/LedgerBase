...
ProcOption()
{
	...
	if(err != YES) return(NOERROR) ;
 	...
 	err = get_pay_param(&pay_param,UPDATE,1,e_mesg);
	if(err < 0) {
  	    DispError((char *)&s_sth,e_mesg);
	    return(err);
	}
...
/*------------------------------------------------------------*/
/* Check to see if it's the last full week of the month. This is 
   done to calculate payroll accruals in the end of month process 
   in ledgerbase. The date is kept in the parameter file of the 
   last full week of the month */
/*------------------------------------------------------------*/
EndofMonthChk()
{
	int	err,month,day,retval;

	day = s_sth.s_paydate%100;
	month = s_sth.s_paydate/100%100;

	/* Check for leap year	*/
	if(month == 02 && (s_sth.s_paydate/10000) % 4 == 0){
		d_month[month-1]++;		
	}

	
	if(day+7 > d_month[month-1] && day != d_month[month-1]) {
		pay_param.pr_week_date = s_sth.s_paydate;
	}

	strcpy(pay_per_it.ppi_code,"BIWEEK");
	pay_per_it.ppi_year = s_sth.s_paydate/10000;
	...
	if(retval != UNDEF) {
		if(day+14 > d_month[month-1] && day != d_month[month-1] &&
		   pay_param.pr_bi_date == 0) {
			pay_param.pr_bi_date = s_sth.s_paydate;
		}
	}
...
Pre_Sched()
{
	...
	pay_period.pp_year = s_sth.s_paydate/10000;
