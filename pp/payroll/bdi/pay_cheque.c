...
#define	ST_FLD		400 
#define	END_FLD		1400 
#define MESG_FLD	1300
#define FUND_FLD	400
#define ACCT_FLD	500
#define CHQNO_FLD	600
#define PRINTER_FLD	700
...
ReadInfo()
{
	...
	for(i = FUND_FLD ; i <= CHQNO_FLD ; i += 100) {
...
CalcAtt()
{
	...
	for(;;){
		...
		if(strcmp(att.at_sick,"Y") == 0)
			bal_sick -= (att_his.eah_hours/att_his.eah_sen_hours);
		...
		if(strcmp(att.at_vacbank, "Y") == 0) &&
			...
			att_his.eah_vacproc[0] = "Y";
			retval = put_emp_at(&att_his, UPDATE, 0, e_mesg);
			if(retval < 0)  {
				fomen(e_mesg);
				get();
				return(retval);
