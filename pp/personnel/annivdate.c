...
******************************************************************************
About the file:	
	This program will print a list of employees by anniversary date. 
...
PrntHdg()	/* Print heading  */
{
...
#ifdef ENGLISH
	mkln((LNSZ-28)/2,"EMPLOYEE ANNIVERSARY LISTING", 28 );
#else
	...
	mkln(1,"ANNIVERSARY",11);
	mkln(14,"EMPLOYEE",8);
	mkln(34,"EMPLOYEE NAME",13);
	mkln(65,"CC #",4);
	mkln(71,"POSITION",8);
	mkln(80,"CLASS",5);
	mkln(86,"CERTIFICATION",13);
	mkln(100,"LEVEL",5);
	mkln(106,"PERCENTAGE",10);
	if(prnt_line() < 0 )	return(REPORT_ERR);
...
PrntRec()
{
	...
	if(last_ann != emp_rec.em_ann){
		if(emp_rec.em_ann == 0) {
			mkln(1,"NO ANNIV.",9);
		}
		else{
			strcpy(txt_line,month[emp_rec.em_ann-1]);
			mkln(1,txt_line,9);
		}
	}
	...
	mkln(80,emp_rec.em_class,6);
	mkln(89,emp_rec.em_cert,6);
	mkln(101,emp_rec.em_level,2);
	tedit( (char *)&emp_rec.em_perc,"___.___",text_buff, R_DOUBLE ); 
	mkln(108,text_buff,7);
...
PutTmpAdd()
{
	...
	if(sortop2[0] == 'L'){
		if(sched1_flag[0] == 'Y') {
		     tedit( (char *)&emp_sched1.es_cost,"____",e_mesg,R_SHORT ); 
		}
		else {
		     tedit((char *)&emp_rec.em_cc,"____",e_mesg,R_SHORT ); 
		}
		strcpy(tmp_sched1.tm_sortk_1,e_mesg);
	}
	else{
		if(sched1_flag[0] == 'Y') {
			strcpy(tmp_sched1.tm_sortk_1,emp_sched1.es_class);
		}
		else {
			strcpy(tmp_sched1.tm_sortk_1,emp_rec.em_class);
		}
	}	
	tedit( (char *)&emp_rec.em_ann,"__",e_mesg, R_SHORT ); 
	strncat(tmp_sched1.tm_sortk_1,e_mesg,2);	
...
char	*tempindxnm;	/*temparary index file name*/
{
	...
	switch (sortop2[0]){
		case 'B':	/*Name within Bargain unit key*/
			...
			/*Part 2*/
			keysarray[step++] = SHORT;
			keysarray[step++] =  1;
			keysarray[step++] = (char*)&emp_rec.em_ann - (char*)&emp_rec;
			keysarray[step++] = ASCND;
