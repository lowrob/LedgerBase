	...
	if(sortop2[0]=='B'||sortop2[0]=='P'){
		...
		for (;;){
			...
			if(teacher == 1) {
				...
				GetClassDesc();
			}/*end check for emp_sched1 record*/
...
CalcBal()
{
	...
	for(;;){
		...
		if(strcmp(att_rec.at_sick, "Y") == 0){
			if(strcmp(att_rec.at_code, "S11") == 0)
				bal_sic -= .5;
			else if(strcmp(att_rec.at_code, "S12") == 0)
				bal_sic -= 1;
			else
				bal_sic -= 
		...
		if(strcmp(att_rec.at_vac, "Y") == 0){
			if(strcmp(att_rec.at_code, "V11") == 0)
				bal_vac -= .5;
			else if(strcmp(att_rec.at_code, "V12") == 0)
				bal_vac -= 1;
			else
		        	bal_vac -= 
		   		(att_hist.eah_hours/att_hist.eah_sen_hours);
