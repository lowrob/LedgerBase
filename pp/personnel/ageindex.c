...
{	
	...
	for(;;) {
		...
		if((retval=CalcAge())==ERROR)	return(retval);
...
PrntHdg()	/* Print heading  */
{
...
#ifdef ENGLISH
	mkln((LNSZ-18)/2,"EMPLOYEE AGE INDEX", 18 );
#else
	mkln((LNSZ-18)/2,"TRANSLATE        ", 18 );
#endif
	if(prnt_line() < 0 )	return(REPORT_ERR);
	if(prnt_line() < 0 )	return(REPORT_ERR);
	...
	mkln(4,"NUMBER",6);
	mkln(26,"NAME",4);
	mkln(62,"OUT OF DIST        OUT OF PROV",30);
	mkln(99,"YRS  DAYS",9);
	mkln(119,"INDEX",5);
	if(prnt_line() < 0 )	return(REPORT_ERR);
	mkln(62,"YRS  DAYS",9);
...
PrntRec()
{
	...
	mkln(63,txt_line,2);
	tedit((char*)&emp_rec.em_days_out_dist,"_0_.__",txt_line,R_DOUBLE);
	mkln(67,txt_line,6);

	tedit((char*)&emp_rec.em_yrs_out_prov,"0_",txt_line,R_SHORT);
	mkln(81,txt_line,2);
	tedit((char*)&emp_rec.em_days_out_prov,"_0_.__",txt_line,R_DOUBLE);
	mkln(85,txt_line,6);

	tedit((char*)&tot_years,"0_",txt_line,R_SHORT);
	mkln(99,txt_line,2);
	tedit((char*)&tot_days,"_0_.__",txt_line,R_DOUBLE);
	mkln(104,txt_line,6);

	tedit((char*)&age_yrs,"0_",txt_line,R_SHORT);
	mkln(118,txt_line,2);
	tedit((char*)&age_days,"_0_",txt_line,R_SHORT);
	mkln(123,txt_line,3);
	if(prnt_line() < 0 )	return(REPORT_ERR);
	if(prnt_line() < 0 )	return(REPORT_ERR);
...
CalcAge()
{
	...
	tmp_yr1 = (date1/10000)% 100;
	tmp_yr2 = (emp_rec.em_date/10000)% 100;

	age_yrs = tmp_yr1 - tmp_yr2 + emp_rec.em_yrs_out_dist +
		emp_rec.em_yrs_out_prov + tot_years;

	age_days = emp_rec.em_days_out_dist + emp_rec.em_days_out_prov + tot_days;
 
	age_yrs += age_days/barg_unit.b_sick_max;
	age_days = age_days % (short)barg_unit.b_sick_max;
