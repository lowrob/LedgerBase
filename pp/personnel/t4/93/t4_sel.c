...
#define	T_BENIFITS	25000	/* Taxable benifits */
#define	INSURANCE	"GRPLF" /* The code for group life insurance */
#define APRIL		19930400 /* Used for box 52 calc. 1st of April */
...
CalcBox40()
{
	...
	double	doub_sal; /* The salery doubled */
	...
	if(strcmp(class.c_code, emp_rec.em_class) == 0){/*found correct class*/
		if(class.c_yrly_inc == 0 || /* No yearly income set in file */
		  class.c_yrly_inc <= T_BENIFITS) /* Is income high enough */
			t4_adj.ta_other_tax = 0; 
			...
			retval=CalcYTD();
			if(retval<0) return(retval);
			doub_sal = th_salery*2;
			t4_adj.ta_other_tax =
			  ((doub_sal - T_BENIFITS)/doub_sal)*calc_dedytd;
		}
