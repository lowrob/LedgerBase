/******************************************************************************
		Sourcename   : unionrep.c
		System       : 
		Module       :
		Created on   : 92-NOV-16 
		Created  By  : m. galvin 
******************************************************************************
About the file:	
	This program will print a list of employee's sick balance. 
...
PrntHdg()	/* Print heading  */
{
...
#ifdef ENGLISH
	mkln((LNSZ-34)/2,"EMPLOYEE'S SICK BALANCE/BANK/USAGE", 34 );
#else
	...
	mkln(74,"SICK DAYS",9);
	mkln(85,"SICK BANK",9);
	mkln(100,"SICK DAYS",9);
	
	if(prnt_line() < 0 )	return(REPORT_ERR);

	mkln(3,"NUMBER",6);
	mkln(75,"BALANCE",7);
	mkln(85,"BALANCE (days)",14);
	mkln(100,"USED (hours)",14);
...
PrntRec()
{
	...
	if(bal_sick > barg_unit.b_sick_max)
	    tedit((char*)&barg_unit.b_sick_max,"___.0_-",text_buff,R_DOUBLE); 
	else
	...
	for(;;){
		...
		if(retval == EFL ||		
		   (strcmp(att_his.eah_numb, emp_rec.em_numb)) != 0){
			...
			if((strcmp(att_his.eah_code,"S10")==0)||
			  (strcmp(att_his.eah_code,"S20")==0))
				att_total+=att_his.eah_hours;
		}

	tedit((char*)&emp_rec.em_sic_bk,"___.0_-",text_buff,R_DOUBLE);
	mkln(85,text_buff,7);

	sprintf(text_buff,"%3.2f",att_total);
	mkln(100,text_buff,7);
