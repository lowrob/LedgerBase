/*-----------------------------------------------------------------------
Source Name: chg_emp.c
Created  On: December 12, 1991.
Created  By: Eugene Roy.

DESCRIPTION:
	Program to Change the Employee Number.
...
#define	MOD_DATE	"12-DEC-91"		/* Program Last Modified */
...
#define	SCR_NAME	"chg_emp"	/* PROFOM screen Name */

/* PROFOM Screen STH file */

/* Field PROFOM numbers */

#define	OPTION_FLD	1200	/* Option: */
#define	KEY_START	400	/* Key Start Field */
#define	KEY_END		1100	/* Key Start Field */
#define	END_FLD		1400	/* Last Field of the screen */
#define	EMP_FLD		400	/* Employee Cd: */
#define	NEW_EMP_FLD	1100	/* Employee Cd: */
...
struct	s_struct	{
	...
	char	s_new_emp[13];	/* STRING XXXXXXXXXX Field 400 */
...
Validate()
{
	...
	case	EMP_FLD	:	/* Employee #: */
		...
		break ;
	case	NEW_EMP_FLD :	/* Employee #: */
		Right_Justify_Numeric(s_sth.s_new_emp,
					(sizeof(emp_rec.em_numb)-1));
		STRCPY(emp_rec.em_numb,s_sth.s_new_emp) ;
		err = get_employee(&emp_rec, BROWSE, 0, e_mesg) ;
		if(err == 0) {
		  DispError((char *)&s_sth,"Employee Number Already Exists") ;
		  s_sth.s_new_emp[0] = LV_CHAR;
		  return(ERROR) ;
		}
...
Chg_Emp()
{
	...
	err = put_employee(&emp_rec,ADD,e_mesg) ;
	if(err < 0) {
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(err);
	...
	err = put_employee(&emp_rec,P_DEL,e_mesg) ;
	if(err < 0) {
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(err);
	...
	strcpy(emp_emp.ep_numb, s_sth.s_emp);

	err = get_emp_emp(&emp_emp,UPDATE, 0, e_mesg) ;
	if(err < 0 && err != UNDEF) {
		roll_back(e_mesg);
		DispError((char *)&s_sth,e_mesg) ;
		return(ERROR) ;
	...
	if(err != UNDEF) {
		strcpy(emp_emp.ep_numb, s_sth.s_new_emp);

		retval = put_emp_emp(&emp_emp,ADD,e_mesg) ;
		if(retval < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(retval);
		...
		err = put_emp_emp(&emp_emp,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		err = get_n_emp_sched1(&emp_sched1,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(emp_sched1.es_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(emp_sched1.es_numb,s_sth.s_new_emp) ;

		err = put_emp_sched1(&emp_sched1,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_emp_sched1(&emp_sched1,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		err = get_n_emp_sched2(&emp_sched2,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(emp_sched2.es2_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(emp_sched2.es2_numb,s_sth.s_new_emp) ;

		err = put_emp_sched2(&emp_sched2,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_emp_sched2(&emp_sched2,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		err = get_n_emp_extra(&emp_extra,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		strcpy(emp_extra.ee_numb,s_sth.s_new_emp) ;

		err = put_emp_extra(&emp_extra,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_emp_extra(&emp_extra,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		...
		strcpy(emp_earn.en_numb,s_sth.s_new_emp) ;
	...
	for( ; ; ){
		...
		if(strcmp(emp_ins.in_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(emp_ins.in_numb,s_sth.s_new_emp) ;
	...
	for( ; ; ){
		err = get_n_emp_ben(&emp_ben,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(emp_ben.eb_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(emp_ben.eb_numb,s_sth.s_new_emp) ;

		err = put_emp_ben(&emp_ben,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_emp_ben(&emp_ben,P_DEL,e_mesg) ;
	...
	for( ; ; ){
		err = get_n_emp_ded(&emp_ded,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(emp_ded.ed_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(emp_ded.ed_numb,s_sth.s_new_emp) ;

		err = put_emp_ded(&emp_ded,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_emp_ded(&emp_ded,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		err = get_n_emp_bhis(&emp_bh,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(emp_bh.ebh_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(emp_bh.ebh_numb,s_sth.s_new_emp) ;

		err = put_emp_bhis(&emp_bh,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
		...
		err = put_emp_bhis(&emp_bh,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		err = get_n_emp_dhis(&emp_dh,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(emp_dh.edh_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(emp_dh.edh_numb,s_sth.s_new_emp) ;

		err = put_emp_dhis(&emp_dh,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_emp_dhis(&emp_dh,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		...
		if(strcmp(emp_loan.el_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(emp_loan.el_numb,s_sth.s_new_emp) ;

		err = put_emp_loan(&emp_loan,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_emp_loan(&emp_loan,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		err = get_n_emp_lhis(&emp_lh,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(emp_lh.elh_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(emp_lh.elh_numb,s_sth.s_new_emp) ;

		err = put_emp_lhis(&emp_lh,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_emp_lhis(&emp_lh,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		err = get_n_emp_garn(&emp_garn,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(emp_garn.eg_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(emp_garn.eg_numb,s_sth.s_new_emp) ;

		err = put_emp_garn(&emp_garn,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_emp_garn(&emp_garn,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		err = get_n_emp_ghis(&emp_gh,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(emp_gh.egh_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(emp_gh.egh_numb,s_sth.s_new_emp) ;

		err = put_emp_ghis(&emp_gh,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_emp_ghis(&emp_gh,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		err = get_n_teach_ass(&teach_ass,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		strcpy(teach_ass.tc_numb,s_sth.s_new_emp) ;

		err = put_teach_ass(&teach_ass,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_teach_ass(&teach_ass,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		err = get_n_teach_qual(&teach_qual,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		strcpy(teach_qual.tq_numb,s_sth.s_new_emp) ;

		err = put_teach_qual(&teach_qual,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_teach_qual(&teach_qual,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		err = get_n_emp_at(&emp_att,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(emp_att.eah_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(emp_att.eah_numb,s_sth.s_new_emp) ;

		err = put_emp_at(&emp_att,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_emp_at(&emp_att,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		...
		strcpy(emp_att.eah_teach,s_sth.s_new_emp) ;

		err = put_emp_at(&emp_att,UPDATE,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		...
		strcpy(time_rec.tm_numb,s_sth.s_new_emp) ;
	...
	for( ; ; ){
		...
		strcpy(time_rec.tm_teach,s_sth.s_new_emp) ;
	...
	time_his.tmh_week = 0;
	time_his.tmh_fund = 0;
	time_his.tmh_adj[0] = '\0';
	time_his.tmh_class[0] = '\0';
	time_his.tmh_earn[0] = '\0';
	time_his.tmh_trans[0] = '\0';
	time_his.tmh_exp[0] = '\0';
	flg_reset(TIME_HIS);
	
	for( ; ; ){
		err = get_n_time_his(&time_his,UPDATE, 0, FORWARD, e_mesg) ;
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(time_his.tmh_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(time_his.tmh_numb,s_sth.s_new_emp) ;

		err = put_time_his(&time_his,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
		...
		err = put_time_his(&time_his,P_DEL,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		...
		strcpy(pp_earn.pe_numb,s_sth.s_new_emp) ;

		err = put_pp_earn(&pp_earn,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		...
		strcpy(pp_ben.pb_numb,s_sth.s_new_emp) ;

		err = put_pp_ben(&pp_ben,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		...
		strcpy(pp_ded.pd_numb,s_sth.s_new_emp) ;

		err = put_pp_ded(&pp_ded,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		...
		strcpy(pp_loan.pc_numb,s_sth.s_new_emp) ;

		err = put_pp_loan(&pp_loan,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ; ){
		...
		strcpy(pp_garn.pg_numb,s_sth.s_new_emp) ;

		err = put_pp_garn(&pp_garn,ADD,e_mesg) ;
		if(err < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(err);
	...
	for( ; ;){
		...
		strcpy(jr_ent.jr_emp_numb,s_sth.s_new_emp);

		err = put_jr_ent(&jr_ent,ADD,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
	...
	for( ; ;){
		err = get_n_jrh_ent(&jr_ent_his,UPDATE,1,FORWARD,e_mesg);
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(jr_ent_his.jrh_emp_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}
		...
		err = put_jrh_ent(&jr_ent_his,P_DEL,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
		strcpy(jr_ent_his.jrh_emp_numb,s_sth.s_new_emp);

		err = put_jrh_ent(&jr_ent_his,ADD,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
	...
	for( ; ;){
		err = get_n_chq_ms_ass(&chq_mess_ass,UPDATE,0,FORWARD,e_mesg);
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(chq_mess_ass.ma_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(chq_mess_ass.ma_numb,s_sth.s_new_emp);

		err = put_chq_ms_ass(&chq_mess_ass,ADD,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
		...
		err = put_chq_ms_ass(&chq_mess_ass,P_DEL,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
	...
	err = get_rg_pen_adj(&reg_pen_adj,UPDATE,0,e_mesg);
	if(err < 0 && err != UNDEF) {
		roll_back(e_mesg);
		DispError((char *)&s_sth,e_mesg) ;
		return(ERROR) ;
	...
	if(err != UNDEF) {
		strcpy(reg_pen_adj.rpa_numb,s_sth.s_new_emp);

		retval = put_rg_pen_adj(&reg_pen_adj,ADD,e_mesg);	
		if(retval < 0) {
	 		DispError((char *)&s_sth,e_mesg);
	  		return(ERROR);
		}
		...
		err = put_rg_pen_adj(&reg_pen_adj,P_DEL,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
	...
	for( ; ;){
		err = get_n_aud_pay(&aud_pay,UPDATE,0,FORWARD,e_mesg);
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(aud_pay.aud_emp, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(aud_pay.aud_emp,s_sth.s_new_emp);

		err = put_aud_pay(&aud_pay,ADD,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
		...
		err = put_aud_pay(&aud_pay,P_DEL,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
	...
	retval = get_ec(&ec_rec,BROWSE,0,e_mesg) ;
	...
	if(retval != UNDEF){
		strcpy(ec_rec.ec_numb,s_sth.s_new_emp);

		retval = put_ec(&ec_rec,ADD,e_mesg);	
		if(retval < 0) {
			DispError((char *)&s_sth,e_mesg);
			return(ERROR);
		}
		...
		err = put_ec(&ec_rec,P_DEL,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
	...
	for( ; ;){
		err = get_n_chq_reg(&chq_reg,UPDATE,1,FORWARD,e_mesg);
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(chq_reg.cr_emp_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}
		...
		err = put_chq_reg(&chq_reg,P_DEL,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
		strcpy(chq_reg.cr_emp_numb,s_sth.s_new_emp);

		err = put_chq_reg(&chq_reg,ADD,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
	...
	for( ; ;){
		err = get_n_man_chq(&man_chq,UPDATE,0,FORWARD,e_mesg);
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(man_chq.mc_emp_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(man_chq.mc_emp_numb,s_sth.s_new_emp);

		err = put_man_chq(&man_chq,ADD,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
		...
		err = put_man_chq(&man_chq,P_DEL,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
	...
	if(err != UNDEF) {
		...
		strcpy(roe.ro_emp_numb,s_sth.s_new_emp);

		retval = put_roe(&roe,ADD,e_mesg);	
		if(retval < 0) {
	 		DispError((char *)&s_sth,e_mesg);
	  		return(ERROR);
		}
	...
	for( ; ;){
		err = get_n_tmp_sched1(&tmp_sched1,UPDATE,0,FORWARD,e_mesg);
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(tmp_sched1.tm_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(tmp_sched1.tm_numb,s_sth.s_new_emp);

		err = put_tmp_sched1(&tmp_sched1,ADD,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
		...
		err = put_tmp_sched1(&tmp_sched1,P_DEL,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
	...
	for( ; ;){
		err = get_n_emp_sen(&emp_sen,UPDATE,0,FORWARD,e_mesg);
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		strcpy(emp_sen.esn_numb,s_sth.s_new_emp);

		err = put_emp_sen(&emp_sen,ADD,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
		...
		err = put_emp_sen(&emp_sen,P_DEL,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
	...
	for( ; ;){
		err = get_n_tmp_sen(&tmp_sen,UPDATE,0,FORWARD,e_mesg);
		if(err < 0) {
			roll_back(e_mesg);
			if(err == EFL) break;
			DispError((char *)&s_sth,e_mesg) ;
		...
		if(strcmp(tmp_sen.tsn_numb, s_sth.s_emp) != 0){ 
			roll_back(e_mesg);
			break;
		}

		strcpy(tmp_sen.tsn_numb,s_sth.s_new_emp);

		err = put_roe(&roe,ADD,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
		...
		err = put_tmp_sen(&tmp_sen,P_DEL,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
	...
	err = get_t4_adj(&t4_adj,UPDATE,0,e_mesg);
	if(err < 0 && err != UNDEF) {
		DispError((char *)&s_sth,e_mesg) ;
		return(ERROR) ;
	}
	...
	if(err != UNDEF) {
		strcpy(t4_adj.ta_numb,s_sth.s_new_emp);

		retval = put_t4_adj(&t4_adj,ADD,e_mesg);	
		if(retval < 0) {
	 		DispError((char *)&s_sth,e_mesg);
	  		return(ERROR);
		}
		...
		err = put_t4_adj(&t4_adj,P_DEL,e_mesg);	
		if(err < 0) {
		 	DispError((char *)&s_sth,e_mesg);
		  	return(ERROR);
		}
	...
	if(retval != UNDEF){
		strcpy(tmp_emp.tem_numb,s_sth.s_new_emp);
	...
	for( ; ;){
		...
		strcpy(tmp2_emp.tem2_numb,s_sth.s_new_emp);

		retval = put_tmp2_emp(&tmp2_emp,ADD,e_mesg);	
		if(retval < 0) {
	 		DispError((char *)&s_sth,e_mesg);
	  		return(ERROR);
		}

		strcpy(tmp2_emp.tem2_numb,s_sth.s_emp);
	...
	for( ; ;){
		...
		strcpy(ts_sen.ts_emp_numb,s_sth.s_new_emp);
		...
		strcpy(ts_sen.ts_emp_numb,s_sth.s_emp);
