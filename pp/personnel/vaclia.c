...
#ifdef ENGLISH
	mkln((LNSZ-30)/2,"VACATION PAY LIABILITY LISTING", 30 );
#else
	mkln((LNSZ-30)/2,"TRANSLATE        ", 30 );
#endif
	if(prnt_line() < 0 )	return(REPORT_ERR);
	if(prnt_line() < 0 )	return(REPORT_ERR);
...
PrntRec()
{
	...
	total = D_Roundoff(vacation * hours * salary);
	...
	mkln(50,emp_rec.em_class,6);

	tedit((char*)&vacation,"___,_0_.__-",txt_line,R_DOUBLE);
	mkln(63,txt_line,11);
...
CalcVac()
{
	...
	tmp_vac = emp_rec.em_vac_ent;

	for(i=0;i<12;i++) {
		tmp_vac += emp_rec.em_vac_acc[i];
	}

	strcpy(emp_att.eah_numb,emp_rec.em_numb);
	emp_att.eah_date = pay_param.pr_fisc_st_dt;
	flg_reset(EMP_ATT);
	...
	for(;;) {
		...
		if(emp_att.eah_date > pay_param.pr_fisc_end_dt)
			break;
		...
		if(strcmp(att.at_vac, "Y") == 0){
			tmp_vac -= emp_att.eah_hours / 
				emp_att.eah_sen_hours;
		}
	}

	vacation = D_Roundoff(tmp_vac);
