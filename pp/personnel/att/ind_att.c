...
L.Robichaud	97/04/17	Add code to look for attendance codes of
			S11 and S12.  These two codes will subtract a half
			day (for S11) or a full day (for S12) from  the sick
			accruals.  This is supose to be for short term.
L.Robichaud	97/05/20	Added the code to look for half and full vac 
			days. The codes are V11 and V12 accordingly.
------------------------------------------------------------------------*/
...
/****
Louis: added folloing section to add back on to the vacation bank when
a attendance code that affected the vacation bank is deleted
***/
		...
		if(strcmp(att.at_vacbank,"Y")==0){
			retval = UpdateVacBank(DELETE);
			if(retval < 0) return(ERROR);
		}
/****
Louis: End of section.
***/
...
int	mode;
{
	...
	/* This routine will break up an entry into multiple records */
	for( ; ; ){
		...
		/* Add to add back to vacation bank before deleteing att */
		if(mode == P_DEL){
			strcpy(att.at_code,att_his.eah_code);
			retval = get_att(&att,BROWSE,1,e_mesg);
			if(retval < 0) {
		...
		/* The vacproc seems to be set to Y before this put in add LR*/
		if(mode == ADD) att_his.eah_vacproc[0] = NULL;
...
/****
Louis: added folloing section to take away from the vacation bank when
a attendance code that affected the vacation bank is added
***/
	strcpy(att_his.eah_numb,s_sth.s_emp);
	att_his.eah_date = temppage->Items[item_no].s_datef; 
	/* This routine will break up an entry into multiple records */
	flg_reset(EMP_ATT);
	for( ; ; ){
		retval = get_n_emp_at(&att_his,UPDATE,0,FORWARD,e_mesg) ;
...
/*** 
Check here for a break if the date excedes the to date. and the employee
number has not changed.
****/
		if(strcmp(att_his.eah_numb, s_sth.s_emp))
			break;
		if(att_his.eah_date > temppage->Items[item_no].s_datet)
			break;
...
/**** Louis R.
	Added check not to allow changes on items that affect the vacation bank
****/
		
		strcpy(att.at_code,CurPage->Items[s_sth.s_field-1].s_code);
		retval = get_att(&att,BROWSE,1,e_mesg);
		if(retval < 0) {
			fomer("Attendance Code Does not Exist");
			return(ERROR);
		...
		if(strcmp(att.at_vacbank,"Y")==0){
			fomer("Item affects the vacation bank and can only be deleted and re-added");
			continue;
		}
/***End of check L.R **/
	...
	for(;;) {
		...
		if(strcmp(att.at_sick,"Y") == 0){
			if(strcmp(att.at_code, "S11") == 0)
				...
				real_bal_sick -= 
				(s_sth.s_items[item_no].s_hours / sen_hrs);
...
/***** 
Added by Louis R. to update the employee vacation bank on adds and deletes
************************************************************************/
...
int	mode;
{
	...
	if(mode == DELETE){
		/* add amount back to vacation bank */
		...
		/* remove amount from to vacation bank */
		if(strcmp(att_his.eah_vacproc, "Y")){
			emp_rec.em_vac_bk -= att_his.eah_hours / 
				att_his.eah_sen_hours;
	...
	retval = commit(e_mesg) ;
	if(retval < 0) {
		DispError((char *)&s_sth,"ERROR: Commiting Records");
		DispError((char *)&s_sth,e_mesg);
		roll_back(e_mesg);
		return(retval);
	}
