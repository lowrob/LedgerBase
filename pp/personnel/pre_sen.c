...
GetSen()
{
	...
	if(month != month2){
	  /* Louis: The folloing 5 lines are for a leap year calculation */
	  this_year = get_date() / 10000; /* Get the year */
	  dblvalue = this_year / 4; /* Divide by 4 and get the number and dec*/
	  wholevalue = dblvalue; /* Get the whole number without dec */
	  /* If it is divisable evenly with no dec then it is a leap year */
	  if(dblvalue - wholevalue == 0){
