...
typedef	struct	{	/* Start Fld 900, Endfld 5300 and Step 300 */
	...
	short	s_num_days;	/* 3950 NUMERIC 9 */
...
BuildList()
{
	...
	for( ; ; ) {
		...
		if(strcmp(earnings.ea_code,"    07") == 0 ||
		   strcmp(earnings.ea_code,"    12") == 0) {
			CurPage->Items[idex].s_num_days = time_rec.tm_num_days;
		}
		else {
			CurPage->Items[idex].s_num_days = HV_SHORT;
		}
...
BuildEmp()
{
	...
	for( ; ; ) {
		...
		CurPage->Items[i].s_num_days = HV_SHORT;
...
int	mode;
{
	...
	if(temppage->Items[item_no].s_num_days == HV_SHORT) {
		time_rec.tm_num_days = 0;
	}
	else {
		time_rec.tm_num_days = temppage->Items[item_no].s_num_days;
	}
...
*/
	...
	if(add_flg == 1){
	  ...
	  if ( WriteFields((char *)&s_sth, EARN_FLD, EARN_FLD+100) < 0 )  
		return(-1);
	  if(strcmp(s_sth.s_items[item_no].s_earn,"    07") == 0 || 
	     strcmp(s_sth.s_items[item_no].s_earn,"    12") == 0) {
	  	/* Read Number of Vacation Days	*/
	  	sr.curfld = 1800;
		if(SetDupBuffers(NUM_DAYS_FLD, NUM_DAYS_FLD,1)<0) 
			return(PROFOM_ERR);
		s_sth.s_items[item_no].s_num_days = LV_SHORT;
		retval = ReadCurFld(NUM_DAYS_FLD, NUM_DAYS_FLD,mode);
		if(retval < 0 || retval == RET_USER_ESC) return(retval);
		if (WriteFields((char *)&s_sth,NUM_DAYS_FLD,NUM_DAYS_FLD)<0)
			return(-1);
	  }
...
double 	t_double ;
{
	...
	if( t_char == HV_CHAR ) {
		...
		s_sth.s_items[item_no].s_num_days = t_short ;
