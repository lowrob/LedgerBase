...
PrintRep()
{
	...
	if(s_sth.s_sortop1[0] == 'N'){
		...
		else if(s_sth.s_sortop2[0] == '\0') key_no = 0;
	}
		...
		else if(s_sth.s_sortop2[0] == '\0') key_no = 7;
	}
	...
	for( ; ; ){
		...
		if(pay_param.pr_st_mth == 1){
			emp_sen.esn_date = pay_param.pr_cal_st_dt;
		}
		if(pay_param.pr_st_mth == 2){
			emp_sen.esn_date = pay_param.pr_fisc_st_dt;
		}
		if(pay_param.pr_st_mth == 3){
			emp_sen.esn_date = pay_param.pr_schl_st_dt;
		}
		emp_sen.esn_pos[0] = '\0';
...
PrntHdg()	/* Print heading  */
{
	...
	mkln(1,"Date:",5);
	tedit((char *)&tmp_sen.tsn_date,"____/__/__",txt_buff,R_LONG);
	mkln(7,txt_buff,10);
	if( prnt_line()<0 )	return(REPORT_ERR);
...
PrntTmpSen()
{
	int	retval;
	char	txt_buff[132];
	double	sick_acc, cas_hrs, cas_days, perm_days, cas_totd;
	...
	mkln(1,"Percentage:",11);
	tedit( (char *)&emp_rec.em_perc,"_0_.___",txt_buff,R_DOUBLE);
  	mkln(13,txt_buff,7);
	mkln(21,"Sick Accrual:",13);
	sick_acc = tmp_sen.tsn_sick_acc0 + tmp_sen.tsn_sick_acc1;
	tedit((char *)&sick_acc,"__0_.__",txt_buff,R_DOUBLE);
	mkln(35,txt_buff,7);

	cas_hrs = tmp_sen.tsn_cas_hrs0 + tmp_sen.tsn_cas_hrs1;
	tedit((char *)&cas_hrs,"__0_.__",txt_buff,R_DOUBLE);
	mkln(54,txt_buff,7);
	cas_days = tmp_sen.tsn_cas_days0 + tmp_sen.tsn_cas_days1;
	tedit((char *)&cas_days,"__0_.__",txt_buff,R_DOUBLE);
	mkln(66,txt_buff,7);
	perm_days = tmp_sen.tsn_perm_days0 + tmp_sen.tsn_perm_days1;
	tedit((char *)&perm_days,"__0_.__",txt_buff,R_DOUBLE);
	mkln(78,txt_buff,7);
	cas_totd = tmp_sen.tsn_cas_totd0 + tmp_sen.tsn_cas_totd1;
...
PrntDetl()
{
	...
	mkln(1,"Pay Period:",11);
	tedit( (char *)&tmp_sen.tsn_pp,"__",txt_buff,R_SHORT);
  	mkln(13,txt_buff,2);
