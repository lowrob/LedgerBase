...
#define NO_HLP_WIN	(sr.curfld!=1000 && sr.curfld!=1100 && sr.curfld!=1300\
			&& sr.curfld!=1500 && sr.curfld!=1600)
#define PRESENT		1
#define ABSENT		0
...
struct	tr_struct	{
	...
	short	s_locno;	/* Location number 1300 NUMERIC 999 */
	short	s_period;	/* Period 1400 NUMERIC 99 */
	char	s_dbacc[19];	/* Acct to be debited 1500 STRING */
	char	s_cracc[19];	/* Acct to be credited 1600 STRING */
	double	s_qty;		/* Qty of Xaction 1700 NUMERIC 999999.9999 */
	double	s_amount;	/* Value of Xaction 1800 NUMERIC 99999999.99 */
	char	s_remarks[31];	/* Remarks 1900 STRING */
	char	s_mesg[77];	/* Message field 2000 STRING */
	char	s_resp[2];	/* Response field 2100 STRING */
};
	...
	for( ; ; ){
		...
		switch( s_rec.s_fn[0] ){
			case ADDREC:	/* add a transaction list */
				...
				if( (retval=SetDupBuffers(900,1900,0))<0 )
					return(retval);
			/* enable dup buffers for debit & credit acc field */
				if( (retval=SetDupBuffers(1500,1600,2))<0 )
					return(retval);
				retval = AddRecord();
				roll_back(e_mesg);
				if( retval<0 ) return(-1);
			/* disable dup buffers for debit & credit acc field */
				if( (retval=SetDupBuffers(1500,1900,0))<0 )
					return(retval);
				break;
...
{
	if( SetDupBuffers( 500, 700, 0 )<0 ) /* Disable key dup buffers */
		return(-1);
	/* Read key fields */
	...
	if( (retval=ReadFields(900,1900))<0 )
		return(retval);
	if(retval==ESCAPE){
		if(ClearScreen()<0)	return(-1);
		return(retval);
...
ClearScreen()	/* clear the screen except fn field and screen heading */
{
	...
	****/
	...
	if( WriteFields(500,2100)<0 ) return(-1);
...
fomer("ALloc, CoMmande, ReCu, Renvoi au Fourn, EMissions, RenVoi, RaDiation, AJuster");
				...
				SetTranOption();
			break;
		case 700:	/* transaction serial no */
			break;
		case 900:	/* fund code */
		...
		case 1000:	/* stock item code */
			...
			if( (retval=DispAccounts()<0) || retval==ESCAPE )
				return(retval);
			sr.nextfld = 1100;
			sr.endfld = 1900;
			break; 
		case 1100:	/* Supplier No */
		...
		case 1400:	/* period */
			...
			break;
		case 1500:	/* Debit Account Number */
		case 1600:	/* Credit Account Number */
			if(sr.curfld==1500) 
				str = s_rec.s_dbacc;
			else 
				str = s_rec.s_cracc;
			if( ESC_H ){
			   retval = gl_hlp(s_rec.s_fund, str, &reccod,
					7, 15 );
			   if( retval<0 )	/* error */
					return( retval );
			...
			if( retval!=NOERROR )
				*str=LV_CHAR;
				...
				fomer(gl_rec.desc);
			break;
		case 1700:	/* quantity of transaction */
			...
			if( (retval=CheckQuantity()<0) || retval==ESCAPE )
				return(retval);
			break;
		case 1800:	/* Value of stock */
...
char *mesg;
{
	...
	if( WriteFields(2000,2000)<0 )	return(-1);
	return(0);
}
HideMessage()	/* Hide the message field */
{
	if( FillMsgRespFlds(HIGH)<0 )	return(-1);
	if( WriteFields(2000,2100)<0 ) return(-1);
	return(0);
}
...
FldEdit()
{
	...
	retval = ReadFields(900,1900);	/* Read all non key fields once */
	if( retval<0 || retval==ESCAPE )	
		return(retval);
...
/*  Cathy Burns April 30,1991  This code loses all changes made to the record
...
ProcReceipts()
{
	...
	if( aps_system==PRESENT )
		...
		if((retval= WriteTrhdr(s_rec.s_fund))<0 )	return(retval);
		if((retval= WriteTritems())<0 )			return(retval);

		/* if different funds are being used then we require 
		   two headers */

		if(s_rec.s_fund != s_rec.s_fund2){
			if((retval = WriteTrhdr(s_rec.s_fund2))<0 )
				return(retval);
			if((retval= WriteTritems())<0 )			
				return(retval);
		}
...
ProcIssues()
{
	...
	/* if different funds are being used then we require two headers */
...
WriteTritems()
{
	...
	tr_item.ti_amount = -s_rec.s_amount;
	if(tr_hdr.th_fund == s_rec.s_fund2)
...
GetStKey()	/* Read the header key values, get terminal info ,read rec */
{
	...
	if( SetDupBuffers( 500, 700, 2 )<0 )
		return(-1);
	if( FillKeyFields(LOW)<0 )  return(-1);
	if( (retval=ReadFields(500,700))<0 ) return(-1);
	if( retval==ESCAPE )
		return(ESCAPE);
