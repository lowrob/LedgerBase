/*-----------------------------------------------------------------------
Source Name: bidentry.c
System     : Tendering System.
Created  On: 13th Apr. 92.
Created  By: J. Prescott.

DESCRIPTION:
	Used to enter bids for each Catalogue Number given by a Supplier.
...
#define	MOD_DATE	"13-APR-92"		/* Progran Last Modified */
...
#define	SCR_NAME	"bidentry"	/* PROFOM screen Name */

#define SUPPMAINT	"supplier"	/* supplier maintenance exec. prog. */
...
#define CAT_FLD		0	/* Offset of Catalogue Field in item line */
#define BID_FLD		100	/* Offset of Bid Price Field in item line */
#define DESC_FLD	200	/* Offset of Desc Field in item line */
#define STATUS_FLD	300	/* Offset of Status in item line */

#define ITEM_CAT	1100	/* First Catalogue field in items */
#define ITEM_BID	1200	/* First Bid Price field in items */
#define ITEM_DESC	1300	/* First Description field in items */

/* area.sth - header for C structure generated by PROFOM EDITOR */

typedef	struct	{	/* Start Fld 1100, Endfld 7000 and Step 400 */

	long	s_cat_num;	/* 1100 NUMERIC 9999999 */
	double	s_bid_price;	/* 1200 NUMERIC 9999.9999 */
	char	s_desc[36];	/* 1300 STRING X(25) */
...
typedef struct	{
	...
	char	s_supp_cd[11];	/* 500 NUMERIC 99 */
	double	s_min_dollar;	/* 600 NUMERIC 9999.99 */
...
static	PotBidder pb_rec;	/* Potential Bidder record */
...
int	direction ;
{
	...
	strcpy(bid_rec.bid_supp_cd,s_sth.s_supp_cd);
	inc_str(bid_rec.bid_supp_cd,sizeof(bid_rec.bid_supp_cd)-1,direction);
	if(direction == BACKWARD) {
		bid_rec.bid_cat_num = HV_LONG ;
	}
	else {
		bid_rec.bid_cat_num = 0 ;
	}
	flg_reset(BID);

	err = get_n_bid(&bid_rec,BROWSE,0,direction,e_mesg);
	if(err < 0 && err != EFL) {
		DispError((char *)&s_sth,e_mesg);
		return(err);
	}
	...
	if(err == EFL) {
		...
		strcpy(s_sth.s_supp_cd,bid_rec.bid_supp_cd);
...
ReadKey()
{
	...
	strcpy(temp_supp,s_sth.s_supp_cd);
	temp_min_dollar = s_sth.s_min_dollar;
	...
	if(RET_USER_ESC == i){
		strcpy(s_sth.s_supp_cd,temp_supp);
		s_sth.s_min_dollar = temp_min_dollar;
...
BuildList()
{
	...
	for( ; ; ) {
		retval = get_n_bid(&bid_rec,BROWSE,0,FORWARD,e_mesg);
		if(retval < 0) {
			if(retval == EFL) break;
			DispError((char *)&s_sth,e_mesg);
			return(retval);
		...
		if(strcmp(bid_rec.bid_supp_cd,s_sth.s_supp_cd)!=0){ 
			break;
		}
		if(strcmp(bid_rec.bid_status,AWARDED) == 0) {
			continue;
		}
		...
		CurPage->Items[i].s_cat_num = bid_rec.bid_cat_num;
		CurPage->Items[i].s_bid_price = bid_rec.bid_price;
		strcpy(CurPage->Items[i].s_desc,bid_rec.bid_desc);
...
int	mode;
{
	...
	if(s_sth.s_min_dollar != supplier.s_min_dollar) {
		strcpy(supplier.s_supp_cd,s_sth.s_supp_cd);
		...
		retval = put_supplier(&supplier,UPDATE,e_mesg);
		if(retval < 0) {
			DispError((char *)&s_sth,e_mesg);
			roll_back(e_mesg);
			return(retval);
...
int	mode;
{
	...
	if(mode != ADD) {
		strcpy(bid_rec.bid_supp_cd,s_sth.s_supp_cd) ;
		bid_rec.bid_cat_num = temppage->Items[item_no].s_cat_num;
	...
	strcpy(bid_rec.bid_supp_cd,s_sth.s_supp_cd);
	bid_rec.bid_cat_num = temppage->Items[item_no].s_cat_num;
	bid_rec.bid_tend_num = temppage->Items[item_no].s_cat_num / 100000;
...
int	mode ;
{
	...
	if(RET_USER_ESC == i) {
		if(mode == ADD) {
			InitItem(item_no,HV_CHAR, HV_LONG, HV_DOUBLE);
...
Validation()
{
	...
	if(sr.curfld == KEY_START) {
		...
		item_no = (sr.curfld - ITEM_ST_FLD - CAT_FLD) / STEP;
	...
	case KEY_START:
		...
		if(retval < 0) {
			...
			if(s_sth.s_fn[0] == ADDREC) {
				retval=GetOption((char *)&s_sth,"Supplier not found. Want to add one (Y/N)?", "YN");
#else
				retval=GetOption((char *)&s_sth,"Fournisseur pas retrouve. Desirez-vous en ajouter un (O/N)?", "ON");
#endif
				if(retval < 0 ) return(retval);
				if( retval == YES ){
					retval = execute( SUPPMAINT, Argc, Argv );
					if( retval<0 )	return(retval);
					redraw();
				}
			}
		...
		if(s_sth.s_fn[0] != ADDREC) {
			...
			if(retval == EFL ||
			   strcmp(bid_rec.bid_supp_cd,s_sth.s_supp_cd) !=0) {
				fomer("No Bids Have Been Entered For This Supplier - Please Re-Enter");
				s_sth.s_supp_cd[0] = LV_CHAR;
				return(ERROR);
			}
		}
		...
		break;
	case ITEM_CAT:
		if(CheckCatalogue(s_sth.s_items[item_no].s_cat_num)<0) {
			s_sth.s_items[item_no].s_cat_num = LV_LONG;
			return(ERROR);
		}
		break;
	case ITEM_BID:
		if(s_sth.s_items[item_no].s_bid_price == 0.00) {
#ifdef ENGLISH
			fomer("Bid Price Can not be Zero");
#else
			fomer("Prix d'offre d'achat ne peut pas etre zero");
#endif
			s_sth.s_items[item_no].s_bid_price = LV_DOUBLE;
			return(ERROR);
		}
...
#ifdef ENGLISH
			strcpy(s_sth.s_items[item_no].s_desc,"AS SPECIFIED");
#else
			strcpy(s_sth.s_items[item_no].s_desc,"COMME SPECIFIE");
#endif
		}
		break;
	default:
...
long	cat_num;
{
	...
	retval = get_bid(&bid_rec,BROWSE,0,e_mesg);
	if(retval < 0) {
		if(retval != UNDEF) {
			DispError((char *)&s_sth,e_mesg);
			return(ERROR);
		...
		fomer("Catalogue# Already Bid on by This Supplier");
		return(ERROR);
	}
	...
	if(CurLast != NULL) {
	   for(temppage=FirstPage; temppage!=NULL;temppage=temppage->NextPage) {
	      for(i =0; i< temppage->NoItems; i++) {
			if(temppage->Items[i].s_cat_num == LV_LONG) {
				return(NOERROR);
			}
			if(temppage->Items[i].s_cat_num == cat_num) {
				fomer("Catalogue# Already Entered on Screen");
				return(ERROR);
			}
	      }
	      if(temppage == CurLast) break;
...
WindowHelp()
{
	...
	switch(fld_no){
	case KEY_START:
		retval = supp_hlp(s_sth.s_supp_cd,7,13);
		...
		break;
	case ITEM_CAT:
		retval = catalogue_hlp(&s_sth.s_items[item_no].s_cat_num,7,13);
		if(retval == DBH_ERR) return(retval);
		if(retval >= 0) redraw();
		if(retval == 0) return(ERROR);
		if(retval < 0) return(ERROR);
...
#else
			err = GetOption((char *)&s_sth,
		"O(ui), R(ajouter), M(odifier), E(lim), V(ivif), S(uiv), P(rec), A(nnul)"
		,"OSLA");
#endif
	    }
	    else {
...
ClearScreen()
{
	...
	ret( WriteFields((char *)&s_sth,ITEM_ST_FLD, END_FLD ) );
