...
DESCRIPTION:
	This program purge's catalogue items and prints a report. 
...
PurgeCatalogue()
{
	...
	flg_reset( CATALOGUE );
		...
		if(bycategory[0] == YES && catalogue.cat_num / 100000) {
		...
		if(byitemgrp[0] == YES && catalogue.cat_num / 1000) {
		...
		retval = put_catalogue(&catalogue,P_DEL,e_mesg);
		if(retval < 0) {
			break;
		}
		...
		curr_cat = catalogue.cat_num;
		retval = purge_descs();
		if(retval < 0) {
			break;
		}
		catalogue.cat_num = curr_cat+1;
		flg_reset(CATALOGUE);
	}
	retval = purge_descs();
...
#ifdef ENGLISH
	mkln(28,"PURGED SUPPLIES CATALOGUE",25); 
#else
	mkln(28,"CATALOGUE DE MATERIEL ELIMINE",29);
#endif
	if(prnt_line() < 0) return(ERROR);
	if(prnt_line() < 0) return(ERROR);
...
purge_descs()
{
	int	retval;
	/* purge category */
	catalogue.cat_num = category.categ_num * 100000;
	retval = get_n_catalogue(&catalogue,BROWSE,0,e_mesg);
	if(retval < 0 && retval != EFL) {
		return(retval);
	}
	if(catalogue.cat_num / 100000 != category.categ_num || retval==EFL) {
		retval = get_category(&category,P_DEL,e_mesg);
		if(retval < 0) {
			return(retval);
		}
	}
	...
	if(catalogue.cat_num / 1000 != item_grp.itmgrp_num || retval==EFL) {
		retval = get_itemgrp(&item_grp,P_DEL,e_mesg);
		if(retval < 0) {
			return(retval);
		}
	}
