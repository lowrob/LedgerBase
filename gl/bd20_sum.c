...
~~~~~~~~~~     ~~~~~~~~       ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
F.Tao	       91/01/08       Change Report for District 20. This 
			      budget report does not show account no.
			      for option 6, sort #2 and keys (1,3,11,2,8,9,10).
------------------------------------------------------------------------*/
...
PrntRecord()
{
	...
	if( (SK3 && T3_desc[0] == '\0') ||
		...
		if(RepType == 8 || RepType == 9 || RepType == 10) {
			...
			/* Copy the last 11 charaters */
			if  (rp_sth.s_sort_no != 2 || RepType != 6)
...
char	*desc ;
{
	...
	else {   /* Change made by F.Tao, 01/08/90		*/
		if( key_val != 0 && (rp_sth.s_sort_no != 2 || RepType != 6)){
			mkln(4, " ", 1) ;
			tedit((char*)&key_val, Mask_5, line+cur_pos, R_LONG ) ;
			cur_pos += 5 ;
		}
