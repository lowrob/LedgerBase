...
DESCRIPTION:
	...
	will be called. And that function first calls options selction function
	GetOptions(), which is avialable here. These functions should return
...
~~~~~~~~~~     ~~~~~~~~       ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
F.Tao	       91/01/09	      Fixed so that the program checks the duplicate
			      keys entered.
...
#define MAINFL		GLMAST	/* no main file used */
...
#define	SCR_NAME	"bdgt_rep"	/* PROFOM screen name	*/

#define	ENDFLD	3300	/* PROFOM screen last field */
...
int option;
	for( ; ; ){
		...
		case 2 :	/* With Annual Budget Report */
		case 3 :	/* With Account#s */
			if((ret_cd=CheckAccess(GLMAST,BROWSE,e_mesg))<0)
				break;
			ret_cd = BdgtReport(option) ;
			break;
		case 4 :	/* Distibution - Budget Report */
		case 5 :	/* Distibution - Actual Report */
			if((ret_cd=CheckAccess(GLMAST,BROWSE,e_mesg))<0)
				break;
			ret_cd = BdgtDistribution(option) ;
			break;
		case 6 :	/* Summary - Format A */
		case 7 :	/* Summary - Format B */
		case 8 :	/* Summary - Expenses Report */
		case 9 :	/* Summary - Revenue Report */
		case 10 :	/* Summary - Expenses & Revenue Report */
			if((ret_cd=CheckAccess(GLMAST,BROWSE,e_mesg))<0)
				break;
			ret_cd = BdgtSummary(option) ;
			break;
		default :
			continue ;
		}
