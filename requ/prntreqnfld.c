...
print_item_line()
{
	...
	if(reqitem_rec.bdgt_flag[0] == YES) {
		mkln(24,"*",1);
	}
	...
	for(str_pos=0;str_pos<=strlen(reqitem_rec.desc)-30; ) {
		...
		if(i == str_pos) {
			mkln(26,reqitem_rec.desc+str_pos,30);
        		if(prnt_line() < 0) return(ERROR);
			str_pos += 30;
		}
		else {
			mkln(26,reqitem_rec.desc+str_pos,i-(str_pos-1));
        		if(prnt_line() < 0) return(ERROR);
			i = i - (str_pos-1);
			str_pos += i;
		}	
	}
	mkln(26,reqitem_rec.desc+str_pos,strlen(reqitem_rec.desc)-str_pos);
...
***/
	...
	tedit((char *)&reqitem_rec.unitprice,"___0_.___",txt_line,R_DOUBLE);
	mkln(65,txt_line,9);
