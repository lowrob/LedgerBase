...
#define SCODE1		3700
#define SCODE2		3800
#define ACCOUNT1	3900
#define ACCOUNT2	4000
#define SUPPNAME1	4100
#define SUPPNAME2	4200
#define REQSTAT1	4300
#define REQSTAT2	4400
#define ITMSTAT1	4500
#define ITMSTAT2	4600
#define DATE1		4700
#define DATE2		4800
#define MESSAGE		4900
#define RESPONSE	5000

/* reqrep.sth - header for C structure generated by PROFOM EDITOR */
...
static struct	s_struct	{
	...
	char	s_printer[3];	/* 2800 printer# if option is P */
	short	s_nbrcopies;	/* 2900 page length in no of lines */
	long	s_req_no1;	/* 3000 starting req number */
	long	s_req_no2;	/* 3100 ending req number */
	short   s_fund1;	/* 3200 staring fund number */
	short	s_fund2;	/* 3300 ending fund number */
	short   s_ccno1;	/* 3400 staring cost center number */
	short	s_ccno2;	/* 3500 ending cost center number */
	char	s_title[15];	/* 3600 title for supplier or stock code */
	char	s_scode1[11];	/* 3700 starting supplier/stock no */
	char	s_scode2[11];	/* 3800 ending supplier/stock no */
	char	s_acc1[19];	/* 3900 starting account */
	char	s_acc2[19];	/* 4000 ending account */
	char	s_sname1[25];   /* 4100 starting supplier name */
	char    s_sname2[25];	/* 4200 ending supplier name */
	char	s_reqstat1[2];	/* 4300 starting requisition status */	
	char	s_reqstat2[2];	/* 4400 ending requisition status */	
	char	s_itmstat1[2];	/* 4500 starting item status */	
	char	s_itmstat2[2];	/* 4600 ending item status */	
	long	s_date1;	/* 4700 starting date */
	long	s_date2;	/* 4800 starting date */
	char	s_mesg[77];	/* 4900 message field */
	char	s_resp[2];	/* 5000 response field */
}	s_rec;
...
static int	line[MAXQUERYS];/* for positioning key queries */
static int	multi_line;	    /* line for multi column ranges */
static int	column;		    /* column for multi column ranges */
...
static char	txt[80];
/* this flag is used to see if the shared field is for stock code or */
/* if it is for supplier code.  0 - supplier code  1 - stock code */
static int	supp_stck_validate;
...
short value;
{
	...
	s_rec.s_printer[0] = HL_CHAR(value);
...
short	val1, val2;
{
	s_rec.s_req_no1 = HL_LONG( val1 );
	s_rec.s_req_no2 = HL_LONG( val2 );
	return(0);
}
static
FillSCode( val1, val2 )
...
short val1, val2;
{
	s_rec.s_ccno1 = HV_SHORT * val1;
	s_rec.s_ccno2 = HV_SHORT * val2;
	return(0);
} 
static
...
static
FillReqStat( val1, val2 )
short	val1, val2;
{
	s_rec.s_reqstat1[0] = HL_CHAR( val1 );
	s_rec.s_reqstat2[0] = HL_CHAR( val2 );
	return(0);
}
static
FillItmStat( val1, val2 )
short	val1, val2;
{
	s_rec.s_itmstat1[0] = HL_CHAR( val1 );
	s_rec.s_itmstat2[0] = HL_CHAR( val2 );
	return(0);
}
static
FillSDate( val1, val2 )
...
GetReqRange( reqnbr1, reqnbr2 )
long *reqnbr1, *reqnbr2;
{
	int	retval=0;

	if( line[REQNBR]==0 )	/* no account call made yet */
		line[REQNBR] = STARTLINENUM + call_no++;
	fomca2( REQNO1,1, line[REQNBR], 1 );
	fomca2( REQNO1,2, line[REQNBR], 17 );
	fomca2( REQNO2,1, line[REQNBR], 43 );
	fomca2( REQNO2,2, line[REQNBR], 48 );

	s_rec.s_req_no1 = *reqnbr1;
	s_rec.s_req_no2 = *reqnbr2;
	fomca1( REQNO1, 19, 2 );
	fomca1( REQNO2, 19, 2 );
	sr.nextfld = REQNO1;
	sr.endfld = REQNO2;
	fomud( (char *)&s_rec);
	s_rec.s_req_no1 = LV_LONG;
	s_rec.s_req_no2 = LV_LONG;
	retval = ReadFields( REQNO1, REQNO2 );
	if( retval!=EXIT && retval>=0 ){
		*reqnbr1 = s_rec.s_req_no1;
		*reqnbr2 = s_rec.s_req_no2;
	}
	fomca1( REQNO1, 19, 0 );
	fomca1( REQNO2, 19, 0 );
...
char *supp1, *supp2;
{
	...
	supp_stck_validate = 0;  /* set validation to check supplier code */

	if( line[SUPP_NO]==0 )	/* no account call made yet */
		line[SUPP_NO] = STARTLINENUM + call_no++;

	fomca2( TITLE,2, line[SUPP_NO], 1 );
	fomca2( SCODE1,2, line[SUPP_NO], 17 );
	fomca2( SCODE2,1, line[SUPP_NO], 43 );
	fomca2( SCODE2,2, line[SUPP_NO], 48 );

#ifdef ENGLISH
	STRCPY(s_rec.s_title,"Supplier Code:");
#else
	STRCPY(s_rec.s_title,"Code de fourn:");
#endif

	STRCPY( s_rec.s_scode1, supp1 );
	STRCPY( s_rec.s_scode2, supp2 );
	fomca1( SCODE1, 19, 2 );
	fomca1( SCODE2, 19, 2 );
	sr.nextfld = TITLE;
	sr.endfld = SCODE2;
	...
	retval = ReadFields( TITLE, SCODE2 );
	if( retval!=EXIT && retval>=0 ){
		Right_Justify_Numeric(s_rec.s_scode1,
					sizeof(s_rec.s_scode1)-1);
		Right_Justify_Numeric(s_rec.s_scode2,
					sizeof(s_rec.s_scode1)-1);
		strcpy( supp1, s_rec.s_scode1 );
		strcpy( supp2, s_rec.s_scode2 );
	}
	fomca1( SCODE1, 19, 0 );
	fomca1( SCODE2, 19, 0 );
...
GetStckRange( stckcd1, stckcd2 )
char *stckcd1, *stckcd2;
{
	int	retval=0;

	supp_stck_validate = 1; /* set validation to check stock code */

	if( line[STOCKCODE]==0 )	/* no account call made yet */
		line[STOCKCODE] = STARTLINENUM + call_no++;

	fomca2( TITLE,2, line[STOCKCODE], 1 );
	fomca2( SCODE1,2, line[STOCKCODE], 17 );
	fomca2( SCODE2,1, line[STOCKCODE], 43 );
	fomca2( SCODE2,2, line[STOCKCODE], 48 );

#ifdef ENGLISH
	STRCPY(s_rec.s_title,"Stock Code   :");
#else
	STRCPY(s_rec.s_title,"Code stock   :");
#endif

	STRCPY( s_rec.s_scode1, stckcd1 );
	STRCPY( s_rec.s_scode2, stckcd2 );
	fomca1( SCODE1, 19, 2 );
	fomca1( SCODE2, 19, 2 );
	sr.nextfld = TITLE;
	sr.endfld = SCODE2;
	...
	retval = ReadFields( TITLE, SCODE2 );
	if( retval!=EXIT && retval>=0 ){
		strcpy( stckcd1, s_rec.s_scode1 );
		strcpy( stckcd2, s_rec.s_scode2 );
	}
	fomca1( SCODE1, 19, 0 );
	fomca1( SCODE2, 19, 0 );
...
short *fund1, *fund2;
{
	...
	if((retval = GetPosition(FUND)) < 0) {
		return(retval);
	}
	line[FUND] = multi_line; 
	fomca2(FUND1,1, line[FUND], 1 + (42 * column) );
	fomca2(FUND1,2, line[FUND], 17 + (40 * column) );
	fomca2(FUND2,1, line[FUND], 22 + (39 * column));
	fomca2(FUND2,2, line[FUND], 27 + (39 * column));
...
GetCCRange( ccno1, ccno2 )
short *ccno1, *ccno2;
{
	int	retval=0;

	
	if((retval = GetPosition(CCNO)) < 0) {
		return(retval);
	}
	line[CCNO] = multi_line;
	fomca2(CCNO1,1, line[CCNO], 1 + (42 * column) );
	fomca2(CCNO1,2, line[CCNO], 17 + (40 * column) );
	fomca2(CCNO2,1, line[CCNO], 22 + (39 * column) );
	fomca2(CCNO2,2, line[CCNO], 27 + (39 * column) );
	...
	sr.endfld = CCNO2;
	fomud( (char *)&s_rec);
	s_rec.s_ccno1 = LV_SHORT;
	s_rec.s_ccno2 = LV_SHORT;
	retval = ReadFields( CCNO1, CCNO2 );
	if( retval!=EXIT && retval>=0 ){
		*ccno1 = s_rec.s_ccno1;
		*ccno2 = s_rec.s_ccno2;
	}
...
GetReqStatRange( reqstat1, reqstat2 )
char *reqstat1, *reqstat2;
{
	int	retval=0;

	if((retval = GetPosition(REQSTAT)) < 0) {
		return(retval);
	}
	line[REQSTAT] = multi_line;
	fomca2( REQSTAT1,1, line[REQSTAT], 1 + (42 * column));
	fomca2( REQSTAT1,2, line[REQSTAT], 17 + (40 * column) );
	fomca2( REQSTAT2,1, line[REQSTAT], 22 + (39 * column) );
	fomca2( REQSTAT2,2, line[REQSTAT], 27 + (39 * column) );

	STRCPY( s_rec.s_reqstat1, reqstat1 );
	STRCPY( s_rec.s_reqstat2, reqstat2 );
	fomca1( REQSTAT1, 19, 2 );
	fomca1( REQSTAT2, 19, 2 );
	sr.nextfld = REQSTAT1;
	sr.endfld = REQSTAT2;
	fomud( (char *)&s_rec);
	s_rec.s_reqstat1[0] = LV_CHAR;
	s_rec.s_reqstat2[0] = LV_CHAR;
	retval = ReadFields( REQSTAT1, REQSTAT2 );
	if( retval!=EXIT && retval>=0 ){
		strcpy( reqstat1, s_rec.s_reqstat1 );
		strcpy( reqstat2, s_rec.s_reqstat2 );
	}
	fomca1( REQSTAT1, 19, 0 );
	fomca1( REQSTAT2, 19, 0 );

	return(retval);
}

GetItmStatRange( itmstat1, itmstat2 )
char *itmstat1, *itmstat2;
{
	int	retval=0;

	if((retval = GetPosition(ITMSTAT)) < 0) {
		return(retval);
	}
	line[ITMSTAT] = multi_line;
	fomca2( ITMSTAT1,1, line[ITMSTAT], 1 + (42 * column) );
	fomca2( ITMSTAT1,2, line[ITMSTAT], 17 + (40 * column) );
	fomca2( ITMSTAT2,1, line[ITMSTAT], 22 + (39 * column) );
	fomca2( ITMSTAT2,2, line[ITMSTAT], 27 + (39 * column) );

	STRCPY( s_rec.s_itmstat1, itmstat1 );
	STRCPY( s_rec.s_itmstat2, itmstat2 );
	fomca1( ITMSTAT1, 19, 2 );
	fomca1( ITMSTAT2, 19, 2 );
	sr.nextfld = ITMSTAT1;
	sr.endfld = ITMSTAT2;
	fomud( (char *)&s_rec);
	s_rec.s_itmstat1[0] = LV_CHAR;
	s_rec.s_itmstat2[0] = LV_CHAR;
	retval = ReadFields( ITMSTAT1, ITMSTAT2 );
	if( retval!=EXIT && retval>=0 ){
		strcpy( itmstat1, s_rec.s_itmstat1 );
		strcpy( itmstat2, s_rec.s_itmstat2 );
	}
	fomca1( ITMSTAT1, 19, 0 );
	fomca1( ITMSTAT2, 19, 0 );
...
ClearInquiryArea()
{
	...
	for( i=0; i<MAXQUERYS; i++ )/* so that next time fields can be */
	...
	if(FillFund(HIGH,HIGH)<0)
		return(-1);
	if(FillCCno(HIGH,HIGH)<0)
		return(-1);
	if(FillAccount(HIGH,HIGH)<0 )
		return(-1);
	if(FillReqStat(HIGH,HIGH)<0 )
		return(-1);
	if(FillItmStat(HIGH,HIGH)<0 )
		return(-1);
...
Validate()	/* Validate the values entered by the user */
{
	...
	switch( sr.curfld ){
		...
		case 2700:	/* Filename */
			break;
		case 2800:	/* Printer */
			nbr_printer = atoi(s_rec.s_printer);
			if(nbr_printer  == 0) {
				if(s_rec.s_printer[0] != SPOOL) {
#ifdef ENGLISH
					fomer("Printer Number Must be Between 1 and 99 or S(pool)"); 
#else
					fomer("Numbero d'imprimante doit etre entre 1 et 99 ou B(obiner)"); 
#endif
					s_rec.s_printer[0] = LV_CHAR;
				}
			}
			break;
		case 2900:	/* Number of copies */
			...
			break;
		case REQNO1:	/* start req number */
			break;
		case REQNO2:	/* end req number */
			if( s_rec.s_req_no2 < s_rec.s_req_no1 ){
#ifdef ENGLISH
				fomer("Ending requisition no can't be less than starting requisition no");
#else
				fomer("Numero du req finissant ne peut pas etre moins que le numero du req debutant");
#endif
				s_rec.s_req_no2 = LV_LONG;
			}
			break;
		case SCODE1:      /* starting supplier/stock code */
			if(supp_stck_validate == 0) {
				Right_Justify_Numeric(s_rec.s_scode1,
						sizeof(s_rec.s_scode1)-1);
			}
			break;
		case SCODE2:      /* ending supplier/stock code */
			if(supp_stck_validate == 0) {
				Right_Justify_Numeric(s_rec.s_scode2,
						sizeof(s_rec.s_scode2)-1);
			}
			if(strcmp(s_rec.s_scode2,s_rec.s_scode1 ) <0){
#ifdef ENGLISH
				fomer("Ending Code can't be less than starting Code");
#else
				fomer("Le code finissant ne etre moins le code debutant");
#endif
				s_rec.s_scode2[0] = LV_CHAR;
			}
			break;
...
char	*printer;
{
	...
	STRCPY( s_rec.s_printer, printer );
	fomca1( 2800, 19, 2 );
	sr.nextfld = 2800;
	sr.endfld = 2800;
	fomud( (char *)&s_rec);
	...
	if( retval!=EXIT && retval>=0 )
		strcpy( printer , s_rec.s_printer ) ;
	fomca1( 2800, 19, 0 );
	return( retval );
}
GetNbrCopies( copies )
