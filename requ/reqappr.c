/*-----------------------------------------------------------------------
Source Name: reqappr.c
System     : Accounts Payables.
Created  On: 11th March 91.
Created  By: Jonathan Prescott.

DESCRIPTION:
	Program to Approve Requisitions.

MODIFICATIONS:        

Programmer     YY/MM/DD       Description of modification

L. Robichaud   92/09/23		Fix the approval of stock when there is little
				stock available and approval is being done out
				of order. Put in a status line.
L. Robichaud   92/09/24		Do not allow a negative value for the amount 
				available.
L. Robichaud   92/09/25		Don't show amount if no stock code.
L. Robichaud   92/12/11		Add code to allow requisition interfunding. 
				Also use the stock fund (fund 2) when dealing 
				with stock. The reqhdr and reqitem files both
				had a field added to them to store the second
				stock fund.
L. Robichaud   93/08/02		Fixed the ESC-F for the header edit to put back
				original values when it is used. The Items will
...
#ifdef ENGLISH
	STRCPY(s_sth.s_scrname, "   REQUISITION APPROVAL   ");
#else
...
ReadFunction()
{
	/* Display Fn: options */
#ifdef ENGLISH
	fomer("A(pprove), C(hange), I(nquire), N(ext), P(rev), E(xit)");
#else
	fomer("A(pprouver), C(hanger), I(nterroger), S(uivant), P(recedent), F(in)");
#endif
...
GetUniqueKey()
{
	...
	req_hdr.code = 89999999;
	flg_reset(REQHDR);
	retval = get_n_reqhdr(&req_hdr,BROWSE,0,BACKWARD,e_mesg);
	if(retval < 0) {
		if(retval == EFL) 
			req_hdr.code = 0;
		else {
...
/* Louis added the disaprove line to the if statement */
	if (temppage->Items[item_no].s_itm_stat[0] == 'D' && 
...
/******************
if(err == UNDEF) printf("\nUNDEF record in alloc");
printf("\nThe funds are %d, %d",aloc_rec.st_fund, s_sth.s_stckfund);
printf("\nThe stock codes are %s, %s",aloc_rec.st_code, s_item->s_stck_cd) ;
printf("\nThe location is %d, %d",aloc_rec.st_location ,s_item->s_school) ;
printf("\nThe accounts are %s, %s", aloc_rec.st_expacc, s_item->s_accno) ;
get();
*********************/
...
ApproveHdr()
{
	...
	i = ReadFields(STATUS_FLD, STATUS_FLD,
		KeyAndHdrValidation, HdrAndKeyWindowHelp, 0) ;
	if(PROFOM_ERR == i || DBH_ERR == i) return(i) ;
...
ApproveItems()
{
	...
	for( ; ; ) {
		...
		if(s_sth.s_status[0] == APPROVED) {
			...
			/* Initialize item status field so user can select */
			if(s_sth.s_items[i].s_itm_stat[0] != COMPLETE) {
				s_sth.s_items[i].s_itm_stat[0] = LV_CHAR;
			}
		}
		else {
		...
		err = ReadItem(i,ITM_STAT_FLD,QTY_FLD,UPDATE) ;
		if(PROFOM_ERR == err || DBH_ERR == err) return(err) ;
...
/** Louis; Added this AdjustPrevVals in case amount is increased on app **/
		if(temp < 0.00) { 
			changeflag[0] = YES;
		}
/***
...
/*** Louis Robichaud Need way to check for same gl & stock code
what if I allow the read of same item then subtract current amount 
***/
...
int	item_no;
{
	...
	if(stock_code[0] != '\0' && stock_code[0] != HV_CHAR) {
		...
		/* Calculate requisitioned quantity from file 
		err=stckreq(s_sth.stckfund,s_sth.s_req_no,
			stock_code,&req_qty,e_mesg);
		if(err < 0) {
			fomer(e_mesg); get();
			return(err) ;
...
/*****
			if(err == UNDEF) {
				alloc_qty = stck_rec.st_alloc;
			}
			else {
				alloc_qty = stck_rec.st_alloc - aloc_rec.st_alloc;
			}
****/
		}
/***
