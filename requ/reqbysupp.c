...
*	Source 	: reqbysupp.c 
*
*	Program to Print Requisitions by supplier code using REPORT GENERATOR.
*
...
double  gst_amt, pst_amt;
	...
	req_hdr.funds = fund1;
	STRCPY(req_hdr.supp_cd,suppcd1);
	req_hdr.code = 0;
	...
	for( ; ; ) {
		...
		if(strcmp(req_hdr.supp_cd,suppcd1) < 0 || 
		   strcmp(req_hdr.supp_cd,suppcd2) > 0) {
			continue;
		}
		...
		if(strcmp(prev_suppcd,req_hdr.supp_cd) != 0) {
			STRCPY(prev_suppcd,req_hdr.supp_cd);
