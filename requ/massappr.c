...
/**** This is no longer a entered field 
...
ApproveItems()
{
	int	err, i, setdup_start, setdup_end;
	Page	*tempptr;
	...
	for( ; ; ) {
		...
		err=CheckStockOrderStatus
			(CurPage->Items[i].s_stck_cd,i);
		if(err != NOERROR) return(err);
...
int 	item_no;
{
	...
	for(;;){
		...
		/*This if stops from looking in it's own cost centre for alloc*/
...

{
	...
	/* If this is reached then the item is a stock item */
	...
	amnt_available = stck_rec.st_on_hand - alloc_qty - stck_req_qty;
/**********
printf("\nDid the ReadAlloc and returned a alloc_qty of %lf",alloc_qty);
get();
***********/
	if(amnt_available < 0)
		disp_amnt = 0;
...
/*******
printf("\nAllowing the editing of stock");get();
********/
