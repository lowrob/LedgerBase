...
#define  TOTAL_POS 72       /* Beginning position  of Total amount        */
...
align_req()
{
	...
	mkln(4,tst_string,10);
	mkln(46,tst_string,10);
	...
	for(cnt = 0;cnt < 27; cnt++) {
		mkln(5,"****.****",9); 
		mkln(14,tst_string,6); 
               	mkln(20,tst_string,1);
               	mkln(22,tst_string,1);
               	mkln(23,tst_string,18);
               	mkln(42,tst_string,20);
		mkln(63,"*****.**",9); 
		mkln(TOTAL_POS,"****,***.**",11); 
...
#endif
			if((strlen(reqitem_rec.desc)-1 > 20 && linecnt >= 48)
							|| linecnt > 49) { 
				print_total(0);
				rite_top();
				if(print_header() == ERROR) return(ERROR);
...
print_header()
{
	...
	mkln(15,sch_rec.sc_phone,3);	/* bill to phone number */
	mkln(18,"-",1);			/* with dashes */
	mkln(19,sch_rec.sc_phone+3,3);
	mkln(22,"-",1);
	mkln(23,sch_rec.sc_phone+6,4);
       	if(prnt_line() < 0) return(ERROR);		/* line 8 */

	mkln(15,sch_rec.sc_fax,3);	/* bill to fax number */
	mkln(18,"-",1);			/* with dashes */
	mkln(19,sch_rec.sc_fax+3,3);
	mkln(22,"-",1);
	mkln(23,sch_rec.sc_fax+6,4);
       	if(prnt_line() < 0) return(ERROR);		/* line 9 */

	tedit((char *)&reqhdr_rec.date,"____/__/__",txt_buffer,R_LONG);
	mkln(46,txt_buffer,10);
	...
	mkln(13,supp_rec.s_supp_cd,10);
	if(prnt_line() < 0) return(ERROR);	/* line 11 */
	if(prnt_line() < 0) return(ERROR);	/* line 12 */

	mkln(4,supp_rec.s_name,44);
	mkln(46,sch_rec.sc_name,28);
	if(prnt_line() < 0) return(ERROR);	/* line 13 */

	mkln(4,address[0].add1,30);
	mkln(46,address[1].add1,30);
	if(prnt_line() < 0) return(ERROR);	/* line 14 */
	mkln(4,address[0].add2,30);
	mkln(46,address[1].add2,30);
	if(prnt_line() < 0) return(ERROR);	/* line 15 */
	mkln(4,address[0].add3,30);
	mkln(46,address[1].add3,30);
	if(prnt_line() < 0) return(ERROR);	/* line 16 */
	mkln(4,address[0].postal_cd,10);
	mkln(46,address[1].postal_cd,10);
	if(prnt_line() < 0) return(ERROR);	/* line 17 */

	mkln(9,supp_rec.s_phone,3);	/* supplier to phone number */
	mkln(12,"-",1);			/* with dashes */
	mkln(13,supp_rec.s_phone+3,3);
	mkln(16,"-",1);
	mkln(17,supp_rec.s_phone+6,4);

	mkln(55,sch_rec.sc_phone,3);	/* Ship To to phone number */
	mkln(58,"-",1);			/* with dashes */
	mkln(59,sch_rec.sc_phone+3,3);
	mkln(62,"-",1);
	mkln(63,sch_rec.sc_phone+6,4);
       	if(prnt_line() < 0) return(ERROR);		/* line 18 */

	mkln(9,supp_rec.s_fax,3);	/* Supplier fax number */
	mkln(12,"-",1);			/* with dashes */
	mkln(13,supp_rec.s_fax+3,3);
	mkln(16,"-",1);
	mkln(17,supp_rec.s_fax+6,4);

	mkln(55,sch_rec.sc_fax,3);	/* bill to fax number */
	mkln(58,"-",1);			/* with dashes */
	mkln(59,sch_rec.sc_fax+3,3);
	mkln(62,"-",1);
	mkln(63,sch_rec.sc_fax+6,4);
       	if(prnt_line() < 0) return(ERROR);		/* line 19 */

	mkln(56,reqhdr_rec.attention,15);
...
print_item_line()
{
	...
	tedit((char *)&reqitem_rec.orig_qty,"__0_.____",txt_line,R_DOUBLE);
	mkln(5,txt_line,9);

	mkln(14,reqitem_rec.unit,6); 
	mkln(20,reqitem_rec.tax1,1);		/* print GST taxable always */
	mkln(22,reqitem_rec.tax2,1);		/* print PST taxable always */

	mkln(23,reqitem_rec.acct,18);

	/* subtract 1 extra because of null */
	for(str_pos=0;str_pos<=strlen(reqitem_rec.desc)-21; ) {
		for(i=20+str_pos;i>str_pos;i--) {
			if(reqitem_rec.desc[i] == ' ' )  {
				break;
			}
		}
		if(i == str_pos) {
			mkln(42,reqitem_rec.desc+str_pos,20);
        		if(prnt_line() < 0) return(ERROR);
			str_pos += 20;
		}
		else {
			mkln(42,reqitem_rec.desc+str_pos,i-(str_pos-1));
        		if(prnt_line() < 0) return(ERROR);
			i = i - (str_pos-1);
			str_pos += i;
		}	
	}
	mkln(42,reqitem_rec.desc+str_pos,strlen(reqitem_rec.desc)-str_pos);
