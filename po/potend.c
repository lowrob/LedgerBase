...
J.Prescott     91/05/27       set gst tax default to exempt if supplier
			      does not have a gst registration number.
A.Cormier      92/10/27       Change cost center number from a 2 digits to a
                              4 digits
J McLean       92/11/04	      Changed to allow operator to enter a quantity
			      of zero so the description can relate to the
			      previous item.
S.Whittaker	93/05/28	Updated the rite_audit function to include
				passing the screen structure.
S.Whittaker	93/06/17	If the PO number was already used the po number
				would end up with low values in it.
------------------------------------------------------------------------*/
...
#define	SCR_NAME	"potend"	/* PROFOM screen Name */

#define	PAGESIZE	2		/* No of Items */
#define ALEXISTS	5		/* If po Already Exists */
...
#define	HDR_END_FLD	2400	/* Header End Field */
#define	ITEM_ST_FLD	2500	/* Item 1 Start Field */
#define	END_FLD		9700	/* Last Field of the screen */
#define	STEP		1800	/* NO of fields diff. between 2 items */

#define TOTAL_ST_FLD	6100	/* Totals Start Field */
#define TOTAL_END_FLD	6300	/* Totals End Field */
...
#define GSTMSG_FLD	2000	/* GST message */
#define ATT_FLD		2200    /* Attention: */
#define	PAGENO_FLD	2300	/* Page#: */
#define COLHDG_FLD	2400
...
#define OSQTY_FLD	1600
#define REQNO_FLD	1700

/* po.sth - header for C structure generated by PROFOM EDITOR */

typedef	struct	{	/* Start Fld 2500, Endfld 6000 and Step 1800 */

	short	s_sno;		/* NUMERIC 999 field 2500 */
	char	s_accno[19];	/* STRING XXXXXXXXXXXXXXXXXX Field 2600 */
	long	s_cat_num;	/* NUMERIC 9999999 Field  2700 */
	char	s_stck_cd[11];	/* STRING XXXXXXXXXX Field 2800 */
	char	s_uom[7];	/* STRING XXXXXX Field 2900 */
	char	s_desc[61];	/* STRING X(60) Field 3000 */
	short	s_school;	/* NUMERIC 99 Field 3100 */
	char	s_act_del[4];	/* STRING XXX Field 3200 */
	char	s_subhdg[2];	/* STRING X Field 3300 */
	char	s_gst[2];	/* STRING XX Field 3400 */
	char	s_pst[2];	/* STRING XX Field 3500 */
	double	s_ord_qty;	/* NUMERIC 99999.9999 Field 3600 */
	double	s_unit_cost;	/* NUMERIC 99999.99 Field 3700 */
	double	s_net_amt;	/* NUMERIC 9999999.99 Field 3800 */
	double	s_value;	/* NUMERIC 9999999.99 Field 3900 */
	double	s_pd_amount;	/* NUMERIC 9999999.99 Field 4000 */
	double	s_os_qty;	/* NUMERIC 99999.9999 Field 4100 */
	long	s_reqno;	/* NUMERIC 99999999 Field 4200 */
}	S_item ;
...
typedef struct	{
	...
	char	s_gstmsg[2];	/* STRING X Field 2000 */
	char	s_status[2];	/* STRING X Field 2100 */
	char	s_attention[16];/* STRING X(15) Field 2200 */
	short	s_page_no;	/* NUMERIC 99 Field 2300 */
	char	s_dummy1[4];	/* STRING X Field 2400 */

	S_item	s_items[PAGESIZE] ;	/* Start Fld 2500, End Fld 6000 and
					   Step 1800 */
	double	s_total_po;	/* NUMERIC 9999999.99 Field 6100 */
	double	s_total_itm;	/* NUMERIC 9999999.99 Field 6200 */
	double	s_total_diff;	/* NUMERIC 99999999.99 Field 6300 */
...
static	Last_po		lastpo_rec;	/* Last PO Number Record */
...
GetUniqueKey()
{
	...
	err = ReadFields(KEY_START, KEY_END,
...
int mode;
{
	...
	if(supplier.s_min_dollar != 0.00 && 
	   s_sth.s_po_amt < supplier.s_min_dollar) {
#ifdef ENGLISH
		DispError("Minimum Dollar Requirement Has Not Been Obtained");
#else
		DispError("Besoin minimal d'argent n'a pas ete obtenu");
#endif
		/* This Allows the user to change amounts and accept again */
	}
	...
	if(mode != ADD) {
		err = rite_audit((char*)&s_sth,POHDR,mode,(char*)&po_hdr,(char*)&pre_po,e_mesg);
		if(err==LOCKED) {
			DispError(e_mesg);
			roll_back(e_mesg) ;
			return(LOCKED) ;
...
CheckTotal()
{
	...
	diff = s_sth.s_po_amt - total ;	/* Differnce */
...
int	item_no ;
{
	...
	if(pa_rec.pa_tendering[0] == YES) {
	  if ((po_item.pi_cat_num != temppage->Items[item_no].s_cat_num)
...
/* May 14,1992 - CB Allocation Value and Inventory committed amount should
  not include the rebate.
		Tax_Calculation(gst_tax,pst_tax,
			temppage->Items[item_no].s_net_amt,&tax_cal);
		alloc_amt = tax_cal.gros_amt;
***************/
		alloc_amt = Commit_Calculation(gst_tax,pst_tax,
...
***************/
			aloc_rec.st_value -= Commit_Calculation(gst_tax,pst_tax,
				ctl_rec.rebate, po_item.pi_value, &tax_cal);
		}
	...
	if(pa_rec.pa_tendering[0] == YES && 
		temppage->Items[item_no].s_cat_num != 0) {
		err = UpdateCatalogue(mode,temppage->Items[item_no]);
		if (err < 0) return(err) ;
	}
...
/*-----------------------------------------------------------------------*/
/* Adjust Qty of Old Catalogue if Catalogue Number Changes */
AdjustCatalogue()
{
	int	retval;

	catalogue.cat_num = po_item.pi_cat_num;
	...
	/* Adjust Qty */
	catalogue.cat_qty[0] -= po_item.pi_orig_qty;
...
***************/
		old_value = Commit_Calculation(gst_tax,pst_tax,
		  ctl_rec.rebate, old_value, &tax_cal);
...
/*-----------------------------------------------------------------------*/ 
/* Update Catalogue with new Quantity */
UpdateCatalogue(mode,s_item)
int	mode;	/* mode for po item */
S_item	s_item;
{
	int	retval;

	catalogue.cat_num = s_item.s_cat_num;
	retval = get_catalogue(&catalogue,UPDATE,0,e_mesg);
	if(retval < 0) {
		DispError(e_mesg);
		return(retval);
	...
	catalogue.cat_qty[0] += s_item.s_ord_qty;
...
S_item 	s_item ;
{
	...
	if(s_sth.s_fn[0] == CHANGE && mode != ADD) {
		err = rite_audit((char*)&s_sth,POITEM,mode,(char*)&po_item,(char*)&pre_item,e_mesg);
		if(err==LOCKED) {
			DispError(e_mesg);
			roll_back(e_mesg) ;
			return(LOCKED) ;
...
AddItems()
{
	...
	for( ; ; ) {
		if( PAGESIZE == i) {	/* Page Full */
			...
			for(i-- ; i > 0 ; i--)
			    InitItem(i,HV_CHAR,HV_SHORT,HV_LONG,HV_DOUBLE,ADD) ;
...
int	mode ;
{
	...
	if(ADD == mode) {
		...
		if(item_no!=0 || s_sth.s_page_no != 1) {
		      ...
		      s_sth.s_items[item_no].s_reqno = Prev_reqno;
		      SetDupBuffers(st_fld+REQNO_FLD,st_fld+REQNO_FLD,1);
		}
	...
	InitItem(item_no,LV_CHAR,LV_SHORT,LV_LONG,LV_DOUBLE,mode) ;
	...
	if(RET_USER_ESC == i) {
		if(ADD == mode) {	/* Terminating the ADD */
		      InitItem(item_no,HV_CHAR,HV_SHORT,HV_LONG,HV_DOUBLE,mode);
...
*/
	...
	Prev_reqno = s_sth.s_items[item_no].s_reqno;
...
Page	*pageptr ;
{
	...
	for( ; i < PAGESIZE ; i++ )
		InitItem(i,HV_CHAR,HV_SHORT,HV_LONG,HV_DOUBLE,UPDATE) ;
...
int	mode;
{
...
#ifdef ENGLISH
				fomen("PO already exists - Press Enter To Continue - Contact BDI if this Continues");
#else
				fomen("BC existe deja - Appuyer sur ENTER pour contin. Contact BDI si cela continue");
#endif
				/* s_sth.s_po_no = LV_LONG; */
				get();
				break;  
	...
	case	ENTRYDT_FLD	:	/* Entry Dt: */
		...
		if(s_sth.s_fn[0] == ADDREC || s_sth.s_fn[0] == CHANGE) {
			...
			ret( WriteFields(DUEDT_FLD, DUEDT_FLD) ) ;
			if(mode != UPDATE) {
...
#else
			fomer("Date d'echeance ne peut pas etre avant la date du BC");
#endif
			s_sth.s_due_dt = LV_LONG;
			return(ERROR) ;
		}
...
int	mode ;
{
	...
	switch(fld_no){
	case CTLG_FLD:			/* Catalogue Field */
		if(s_sth.s_items[item_no].s_cat_num == 0) {
			sr.curfld += 100;
			break;
		}
		catalogue.cat_num = s_sth.s_items[item_no].s_cat_num;
		err = get_catalogue(&catalogue,BROWSE,0,e_mesg);
		if(err < 0 ) {
			DispError(e_mesg);
			s_sth.s_items[item_no].s_cat_num = LV_CHAR;
			return(ERROR);
		}
		if(strcmp(catalogue.cat_awd_supp,s_sth.s_supp_cd)!=0) {
			fomer("Supplier Has Not Been Awarded This Item - Please Try Again");
			s_sth.s_items[item_no].s_cat_num = LV_LONG;
			return(ERROR);
		}
		...
		cur_fld += DESC_FLD ;
		STRCPY( s_sth.s_items[item_no].s_desc, catalogue.cat_desc);
		SetDupBuffers(cur_fld,cur_fld,2);

		s_sth.s_items[item_no].s_desc[0] = LV_CHAR ;
		sr.curfld = (ITEM_ST_FLD + (STEP * item_no) ) + CTLG_FLD ;
		sr.endfld = (ITEM_ST_FLD + (STEP * item_no) ) + (STEP - 100) ;

		/** added for unit/cost ****/
		cur_fld = ITEM_ST_FLD + (STEP * item_no) ;
		cur_fld += PRICE_FLD ;
		s_sth.s_items[item_no].s_unit_cost=catalogue.cat_unit_price[0];
		SetDupBuffers(cur_fld,cur_fld,2);
		...
		cur_fld = ITEM_ST_FLD + (STEP * item_no) ;
		cur_fld += UOM_FLD ;
		STRCPY( s_sth.s_items[item_no].s_uom, catalogue.cat_uom);
		SetDupBuffers(cur_fld,cur_fld,2);
	...
	case STCKCD_FLD :		/* Stock Code Field */
		...
		/* Use default from tendering if catalogue number entered */
		if(s_sth.s_items[item_no].s_cat_num != 0 )
			break;
	...
	/* default gst & pst flags to defaults set in parameter file
		...
		if( (s_sth.s_fn[0] == ADDREC) && (mode != UPDATE) ||
			...
			if(pa_rec.pa_dist_gst[0] == YES) {
				/* if no gst registration number for supplier */
				/* set tax default to exempt */
				if(supplier.s_gst_reg[0] != '\0') {
	...
	case QTY_FLD :		/* QTY Fields */
		...
		/*  If the quantity is zero, don't allow  */
		/*  them to enter any more fields.  */

		if(s_sth.s_items[item_no].s_ord_qty == 0) {
			s_sth.s_items[item_no].s_unit_cost = 0.0;
			s_sth.s_items[item_no].s_net_amt = 0.0;
			s_sth.s_items[item_no].s_value = 0.0;
			s_sth.s_items[item_no].s_pd_amount = 0.0;
			s_sth.s_items[item_no].s_os_qty = 0.0;
			s_sth.s_items[item_no].s_reqno = 0;
			break;
		}
...
ItemsWindowHelp()
{
	...
	switch(fld_no){
	case CTLG_FLD :			/* Catalogue Number field */
		err = catalogue_hlp(&s_sth.s_items[item_no].s_cat_num,7, 13 );
		if(err == DBH_ERR) return(err) ;
		if(err >=0 ) redraw();
		if(err < 1) return(ERROR) ;	/* Not Selected */
...
ChangeHdr()
{
	...
	if(RET_USER_ESC == i) {	/* ESC-F */
		...
		*/
		...
		/* Offset to the field where ESC-F Pressed in 'j' */
		fomfp(sr.curfld,&j,&k);
		i =  sizeof(s_struct) - j ;	/* Offset within item */
...
#else
		err = GetOption(
		"O(ui), R(aj art), M(odifier), E(n-tete), T(otal), S(uiv), P(rec), A(nnul)"
		,"ORMTESPA");
#endif
		break ;
...
MakePoItemsList()	/* Make the linked list from PO Items */
{
	...
	for( ; ; ){
		...
		CurPage->Items[i].s_cat_num = po_item.pi_cat_num;
...
InitItem(item_no, t_char, t_short, t_long, t_double, mode)
int	item_no ;
char	t_char ;
short	t_short ;
long	t_long ;
...
int	mode ;
{
	...
	if (pa_rec.pa_tendering[0] == NO) 
		s_sth.s_items[item_no].s_cat_num = HV_LONG ;
	else 
		s_sth.s_items[item_no].s_cat_num = t_long ;
	...
	s_sth.s_items[item_no].s_reqno = t_long;
