...
#define TOTAL_ST_FLD	5500	/* Totals Start Field */
#define TOTAL_END_FLD	5700	/* Totals End Field */
...
typedef	struct	{	/* Start Fld 2500, Endfld 5400 and Step 1500 */
	...
	char	s_uom[7];	/* STRING XXXXXX Field 2800 */
	short	s_school;	/* NUMERIC 99 Field 2900 */
	char	s_desc[61];	/* STRING X(60) Field 3000 */
	char	s_act_del[4];	/* STRING XXX Field 3050 */
...
typedef struct	{
	...
	char	s_dummy1[2];	/* STRING X Field 2400 */

	S_item	s_items[PAGESIZE] ;	/* Start Fld 2500, End Fld 5400 and
					   Step 1500 */
	double	s_total_po;	/* NUMERIC 9999999.99 Field 5500 */
	double	s_total_itm;	/* NUMERIC 9999999.99 Field 5600 */
	double	s_total_diff;	/* NUMERIC 99999999.99 Field 5700 */
...
GetUniqueKey()
{
	...
	po_hdr.ph_code = ctl_rec.last_po + 1 ;
...
int mode;
{
	...
	if(mode == ADD && s_sth.s_po_no < 89999999) {  /* automatic no. po */
		ctl_rec.fund = 1 ;
		err = get_ctl(&ctl_rec,UPDATE, 0, e_mesg) ;
		if(err < 0) {
			DispError(e_mesg);
			return(err) ;
		}
	...
	if(mode == ADD && s_sth.s_po_no < 89999999) {
		ctl_rec.last_po = s_sth.s_po_no ;
		err = put_ctl(&ctl_rec, UPDATE, e_mesg) ;
	}
...
int	item_no ;
{
	...
	if (diff < -(DELTA_AMT) || diff > DELTA_AMT)  {	/* Only if amt chg */
		/* Increase YTD Ordered in supplier */
		supplier.s_ytd_ord += diff ;
	}
	...
	if (s_sth.s_type[0] == BULK || s_sth.s_type[0] == NON_BULK) {
		if ((diff_qty < -(DELTA_QTY) || diff_qty > DELTA_QTY) ||
			...
			if (err < 0) return(err) ;
/*
			err = CheckStock(temppage->Items[item_no].s_stck_cd) ;
			if (err < 0) return(err) ;
			stck_rec.st_on_order += diff_qty ;
			if (stck_rec.st_on_order < DELTA_QTY) 
				stck_rec.st_on_order = 0.00;
			if (s_sth.s_type[0] == NON_BULK) {
				stck_rec.st_alloc += diff_qty ;
...
#ifdef ENGLISH
		err = GetOption(
		"Y(es), A(dd items), E(dit items), H(eader edit), N(ext), P(rev), C(ancel)"
		,"YAEHNPC");
#else
		err = GetOption(
		"O(ui), R(ajouter articles), M(odifier), E(n-tete), S(uiv), P(rec), A(nnul)"
		,"ORMESPA");
#endif
		break ;
