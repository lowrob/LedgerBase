...
#define ATT_FLD		2200	/* Contact Person */
...
typedef	struct	{	/* Start Fld 2500, Endfld 6000 and Step 1800 */
	...
	short	s_sno;		/* NUMERIC 99 field 2500 */
	...
	char	s_subhdg[2];	/* STRING X(60) Field 3300 */
	...
	long	s_reqno;	/* NUMERIC 999999999 Field 4200 */
}	S_item ;
...
typedef struct	{
	...
	S_item	s_items[PAGESIZE] ;	/* Start Fld 2500, End Fld 6100 and
					   Step 1800 */

	double	s_total_po;	/* NUMERIC  Field 6100 */
	double	s_total_itm;	/* NUMERIC  Field 6200 */
	double	s_total_diff;	/* NUMERIC  Field 6300 */
...
int	mode ;
{
	...
	if(po_item.pi_cat_num != 0 && po_item.pi_cat_num != HV_LONG) {
		catalogue.cat_num = po_item.pi_cat_num;
		err = get_catalogue(&catalogue,UPDATE,0,e_mesg);
		if(err < 0) {
			DispError(e_mesg) ;
			return(DBH_ERR) ;
		}
		catalogue.cat_qty[0] -= os_qty;
		err = put_catalogue(&catalogue,UPDATE,e_mesg);
		if(err < 0) {
			DispError(e_mesg) ;
			return(DBH_ERR) ;
		}
...
UpdateWithPoItem()
{
	...
	err = rite_audit((char*)&s_sth,POITEM,P_DEL,(char*)&po_item,(char*)&pre_item,e_mesg);
	if(err == LOCKED) {
		DispError(e_mesg);
		roll_back(e_mesg);
		return(LOCKED);
...
#else
		err = GetOption(
		"O(ui), S(uivant), P(recedent), E(liminer), V(ivifier), A(nnuler)"
		,"OSPERA");
#endif
		break ;
	    case  INQUIRE :		/* Inquire Po */
