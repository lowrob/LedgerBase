...
#define	SCR_NAME	"apprcpt2"	/* PROFOM screen Name */
...
#define	END_FLD		6800	/* Last Field of the screen */
#define	STEP		600	/* NO of fields diff. between 2 items */

#define TOTAL_ST_FLD	6400	/* Totals Start Field */
#define TOTAL_END_FLD	6600	/* Totals End Field */
...
#define	APPAMT_FLD	400
#define	STATUS_FLD	500

/* Total Fields */
#define RECEAMT_FLD	6400
#define APPLAMT_FLD	6500
#define UNAPAMT_FLD	6600

/* Message/Response Fields */
#define MESSAGE_FLD	6700
#define RESP_FLD	6800
...
typedef	struct	{	/* Start Fld 1600, Endfld 5500 and Step 500 */
	...
	double	s_appamt;	/* NUMERIC 99999999.99 Field 2000 */
	char	s_status[4];	/* STRING XXX          Field 2100 */
}	S_item ;
...
int	item_no;
{
	...
	if(strcmp(temppage->Items[item_no].s_status,APPLY_STAT) == 0) {
		...
		rcpt_item.ritm_amount =  temppage->Items[item_no].s_appamt;
...
int	direction ;
{
	...
	s_sth.s_rcpt_amt = rcpt_hdr.rhdr_amount;
	...
	s_sth.s_applamt = rcpt_item_total;
...
MakeInvcItemList()
{
	...
	for( ; ; ) {
		...
		if(rcptexist == 0) {
			...
			CurPage->Items[i].s_appamt = 0;
		}
		else {
			strcpy(CurPage->Items[i].s_status, APPLY_STAT) ;
			CurPage->Items[i].s_appamt = rcpt_item.ritm_amount;
		}
...
/* ANDRE */
			if(strcmp(s_sth.s_items[s_sth.s_field-1].s_status,
			   APPLY_STAT) == 0) {
				s_sth.s_items[s_sth.s_field-1].s_appamt = 0;

				st_fld = ITEM_ST_FLD + (STEP * 
					(s_sth.s_field-1)) + APPAMT_FLD;
				end_fld = ITEM_ST_FLD + (STEP * 
					(s_sth.s_field-1)) + APPAMT_FLD;
			}
			else {
				appamt=s_sth.s_items[s_sth.s_field-1].s_appamt;
				s_sth.s_items[s_sth.s_field-1].s_appamt = 
					LV_DOUBLE;
				...
				/* Get reference number */
				err = ReadFields(st_fld, end_fld,	
					KeyAndHdrValidation,HdrAndKeyWindowHelp,0) ;
				if(PROFOM_ERR == err || DBH_ERR == err) 
					return(err) ;
				if(RET_USER_ESC == err) {	/* ESC-F */
					s_sth.s_items[s_sth.s_field-1].s_appamt=appamt;
			...
			if(strcmp(status,APPLY_STAT) == 0) {
				flag = 1;
				s_sth.s_applamt += 
				   s_sth.s_items[s_sth.s_field-1].s_appamt*flag;
			}
			else {
				flag = -1;
				s_sth.s_applamt += 
				  CurPage->Items[s_sth.s_field-1].s_appamt*flag;
			}


			CurPage->Items[s_sth.s_field-1].s_appamt = 
				s_sth.s_items[s_sth.s_field-1].s_appamt;
...
KeyAndHdrValidation()
{
	...
	if(sr.curfld < ITEM_ST_FLD) {
		...
		fld_no = sr.curfld - ITEM_ST_FLD - (item_no * STEP);
	}


	switch(fld_no){
	case	APPAMT_FLD:
		if(s_sth.s_items[item_no].s_appamt==0) {
			fomer("Applied amount can't be 0");
			s_sth.s_items[item_no].s_appamt = LV_DOUBLE;
			return(ERROR);
		}
		if(s_sth.s_applamt + s_sth.s_items[item_no].s_appamt > 
		   s_sth.s_rcptamt) {
			fomer("Applied amount can't be greather than receipt amount");
			s_sth.s_items[item_no].s_appamt = LV_DOUBLE;
			return(ERROR);
		}
		break;
...
double	t_double ;
{
	...
	s_sth.s_items[item_no].s_appamt   = t_double;
