...
About the program:
	...
	It allows posting of non receipts with online update of the 
	...
	are linked to the corresponding header by sharing a common key.

	A non receipts session comprises of the following actions.
...
#define MOD_DATE	"26-JUN-90"
#define SCREEN_NAME	"nonrecpt"
...
struct	jr_struct	{
	...
	char	s_scrhdg[17];	/* Screen Heading field 200 */
	...
	char	s_mesg[78];	/* STRING X(77) 5100 */
	char	s_resp[2];	/* STRING X 5200 */
};
...
static Counter	current; /* For current line & page of nonreceipts item entry */
...
#ifdef ENGLISH
	STRCPY( s_rec.s_scrhdg,"   NON RECEIPTS   " );
#else
	STRCPY( s_rec.s_scrhdg,"    NON-RECUS    " );
#endif
...
AddRecords()
{
	...
	s_rec.s_tr_code[0] = 'N';	/* non receipts type */
	...
	if( ACCRUALS ){
		...
		s_rec.s_sno++;	/* inc. serial no. on screen and tr_hdr */
		tr_hdr.th_seq_no = s_rec.s_sno;
		if( WriteSession(REVERSED)<0 )	/* write reverse entries */
			return(-1);	/* Updt next period fields of GLmast */
		/* dec. serial no. on screen and tr_hdr to avoid confusion */
...
MakeFreshPage()	/* Add a new node to the linked list */
{
	...
	if( tempptr==NULL ){
#ifdef ENGLISH
             fomen("Internal memory allocation error. Press any key");
#else
...
int mode;		/* ADD_MODE, INQ_MODE */
{
	...
	if( mode==ADD_MODE )
#ifdef ENGLISH
	    DisplayMessage("A(dd), E(dit), D(elete), C(ancel), N(ext), P(rev), R(eactivate), Y(es)");
#else
	    DisplayMessage("R(ajouter), M(odifier), E(limin), A(nnuler), S(uiv), P(rec), V(ivifier), O(ui)");
#endif
...
EditItemFields()	/* Read the field# and allow changes on that field(s) */
{
	...
	for( ; ; ){
		...
		/* Get The Field to Be Modified */
#ifdef	ENGLISH
		DisplayMessage("Press RETURN to terminate Edit");
#else
...
int	entrytype;	/* REVERSED or NORMAL entry */
{
	...
	if( WriteHeader()<0 ){	/* Write the header record in gltrhdr file */
		roll_back(e_mesg) ;
		return(-1);
	}
...
int	direction ;
{
	...
	if((tr_hdr.th_create[0] == 'E' && tr_hdr.th_type[0] == 'N')
	 	|| retval==EFL || tr_hdr.th_fund != s_rec.s_fund) {
			break ;
	}
	}
#ifndef	ORACLE
...
GetHeaderKey()	/* Read the header key values, get terminal info ,read rec */
{
	...
	s_rec.s_sno = LV_LONG ;
	fund_default();
	if( (retval = ReadFields(KEY_START,KEY_END,HdrValidation))<0   || (retval==EXIT)  )  
		return(retval);
	tr_hdr.th_fund = s_rec.s_fund;
	tr_hdr.th_reccod = s_rec.s_reccod;
	tr_hdr.th_seq_no = s_rec.s_sno;
...
BuildList()	/* Read item by item from file and build list */
{
	...
	if( headptr==NULL ){
#ifdef ENGLISH
                 fomen("No Transaction Items to Display. Press any Key");
#else
