...
DESCRIPTION:
	This Program prints the Payment De-Selection Process Report.
...
CloseReport()
{
	if(pgcnt) {		/* If report is started, ie deselection done */
		PrintCloseTotals() ;
#ifndef	SPOOLER
		rite_top() ;
#endif
...
int	flg ;
{
...
#ifdef ENGLISH
	mkln(52, "PAYMENT  DE-SELECTION  REPORT", 30) ;
#else
	mkln(51, "RAPPORT DES RETRAITS DE PAIEMENT", 33) ;
#endif
	if(prnt_line() < 0) return(ERROR);
	...
	mkln(50, "Trans  Genre Per  Trans      Echeance  /Balance trans", 53) ;
	mkln(111, "Prise           Net  ", 21) ;
#endif
	if(prnt_line() < 0) return(ERROR);
...
/*------------------------------------------------------------*/
/* Print De-selected payment details */
...
PrintCloseTotals()
{
	...
	/* Print totals */
#ifdef ENGLISH
	mkln(1, "No of Payments Cancelled", 30) ;
#else
	mkln(1, "Nombre de paiements annules", 33) ;
#endif
	mkln(37, " ", 1);
